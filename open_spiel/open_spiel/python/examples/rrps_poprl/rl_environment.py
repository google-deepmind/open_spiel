...
class Environment(object):
  ...
  def set_prediction_label(self, label):
  ...
  def get_time_step(self):
    ...
    for player_id in range(self.num_players):
      ...
      observations["info_state"].append(
          self._state.observation_tensor(player_id)
    ...
    observations["prediction_label"] = self._prediction_label
    ...
    if self.is_turn_based:
      ...
      for p in range(len(actions)):
        ...
        if legal_actions and actions[p] not in legal_actions:
          raise RuntimeError(
              f"step() by player {p} called on illegal "
              + f"action: {actions[p]}"
          )
  ...
  def step(self, actions):
    ...
    """
    assert (
        len(actions) == self.num_actions_per_step
  ...
  def reset(self):
    ...
    observations = {
        ...
        "current_player": [],
        "serialized_state": [],
        "prediction_label": self._prediction_label,
    }
    for player_id in range(self.num_players):
      observations["info_state"].append(
          self._state.observation_tensor(player_id)
  ...
  def _sample_external_events(self):
    ...
    while self._state.is_chance_node() or (
        self._state.current_player() == pyspiel.PlayerId.MEAN_FIELD
    ):
  ...
  def observation_spec(self):
    ...
    return dict(
        info_state=tuple([
            ...
            if self._use_observation
            else self._game.information_state_tensor_size()
        ]),
  ...
  def is_turn_based(self):
    return (
        self._game.get_type().dynamics == pyspiel.GameType.Dynamics.SEQUENTIAL
    ) or (
        self._game.get_type().dynamics == pyspiel.GameType.Dynamics.MEAN_FIELD
    )
