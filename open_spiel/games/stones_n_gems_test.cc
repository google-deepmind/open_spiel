...
void BasicStonesNGemsTests() {
  testing::LoadGameTest("stones_n_gems");
  testing::ChanceOutcomesTest(*LoadGame("stones_n_gems"));
  testing::RandomSimTest(*LoadGame("stones_n_gems"), 100);
}
...
void ExtendedStonesNGemsTest() {
  ...
  std::shared_ptr<const Game> game = LoadGame(
      "stones_n_gems", {{"magic_wall_steps", GameParameter(20)},
                        {"blob_chance", GameParameter(50)},
                        {"blob_max_percentage", GameParameter(0.25)},
                        {"rng_seed", GameParameter(1)},
                        {"grid", GameParameter(std::string(kTestDefaultGrid))},
  ...
  state->ApplyAction(stones_n_gems::Directions::kDown);
  SPIEL_CHECK_FALSE(state->IsTerminal());
  SPIEL_CHECK_EQ(state->PlayerReward(0), 10);
  SPIEL_CHECK_EQ(state->PlayerReturn(0), 10);
  SPIEL_CHECK_TRUE(state->IsChanceNode());
  state->ApplyAction(stones_n_gems::Directions::kNone);
  ...
  SPIEL_CHECK_FALSE(state->IsChanceNode());
  state->ApplyAction(stones_n_gems::Directions::kRight);
  SPIEL_CHECK_FALSE(state->IsTerminal());
...
int main(int argc, char** argv) {
  open_spiel::stones_n_gems::BasicStonesNGemsTests();
  open_spiel::stones_n_gems::BasicStonesNGemsTestsWithParams();
  open_spiel::stones_n_gems::ExtendedStonesNGemsTest();
}
