...
// Then started with the player on the dealer's left, the game goes counterclockwise
...
// And then place this card on the discard pile top for the next player to match.
// A player can play an 8 as a wild card, however, at anytime.
// If it does so then a color needs to be nominated for the next player to match.
// A player can also decide to draw cards from the dealer deck.
// Notice that it is the only action available if it does not have a available 
// card to play at its turn. But it doesn't prevent the player to draw cards even if
// it has playable cards.
// However, the maximum number of cards a player can draw at its turn is bounded.
// If a player plays a card, it cannot draw at the current turn anymore.
// The game ends if a player has played all of its card.
// The other players are penalized according to the cards on their hand.
// That is, -50 for each 8, -10 for each court card, and -{face value} for others.
//
//
...
// However, it admits stacking. That is, if the next player has 2, it can play it.
// And then the next player after it should draw 4 cards unless it plays draw 2 as well, etc.
// If a player starts to draw in this case, it must draw all the cards and then passes.
// I.e., if it draws a draw 2 card during the drawing, it is not allowed to play it.
//
// If the first card turned face up by the dealer is a special card,
// then it acts as if the dealer plays the card.
//
// If reshuffle = true, then the discard pile got reshuffle and become the new dealer card
// once exhausted.
//
...
// action id 58, 59, ...., 58 + num_player-1: decide the dealer.
//
...
class CrazyEightsState: public State{
 public:
  CrazyEightsState(std::shared_ptr<const Game> game, int num_players, int max_draw_cards, bool use_special_cards, bool reshuffle);
  ...
  private:
    ...
    // if num_passes = num_player_ + 1, then the game ends
    int num_passes_ = 0;
...
class CrazyEightsGame: public Game{
  public:
    ...
    int NumDistinctActions() const override {return kDecideDealerActionBase + num_players_;}
    int MaxChanceOutcomes() const override {return kDecideDealerActionBase + num_players_;}
    std::unique_ptr<State> NewInitialState() const override {
      return absl::make_unique<CrazyEightsState>(shared_from_this(), num_players_, max_draw_cards_, use_special_cards_, reshuffle_);
    }
    int NumPlayers() const override {return num_players_;}
    double MinUtility() const override {return -504 * (num_players_ > 5? 2: 1);}
    double MaxUtility() const override {return 0.0;}
    std::vector<int> ObservationTensorShape() const override {
      int num_decks = num_players_ > 5? 2: 1;
      if(!use_special_cards_){
        return {(num_decks + 1) * kNumCards + kNumCards + kNumSuits + (num_decks * kNumCards + 1) * (num_players_-1)};
      } else return {(num_decks + 1) * kNumCards + kNumCards + kNumSuits + (num_decks * kNumCards + 1) * (num_players_-1) + 1};
