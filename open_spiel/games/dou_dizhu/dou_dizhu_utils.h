...
// player 0, 1 passes, 2 bids 1, then 0 passes, 1 bids 2, 2 passes, 0 bids 3, 1 & 2 passes
inline constexpr int kMaxAuctionLength = 9;

// the maximum/minimum utility is achieved if the players play all 13 bombs alternatively
// and dizhu bid maximum bids
...
inline constexpr int kObservationTensorSize = 
    2* ((kNumRanks - 2) * (kNumSuits + 1) + 2 * 2) 
     + kNumPlayers + kNumPlayers + kNumRanks;
...
inline constexpr int kAirplaneWithPairActionBase = kAirplaneWithSoloActionBase + 22588;
...
struct ChainOnlyHandParams{
  ...
  int chain_length;
  ChainOnlyHandParams(int h, int n, int l): chain_head(h), num_cards_per_rank(n), chain_length(l){}
  ChainOnlyHandParams() {}
};
...
struct TrioCombParams{
  ...
  int kicker_id;
  TrioCombParams(int head, int length, KickerType k, int k_id): chain_head(head), 
  chain_length(length), kicker_type(k), kicker_id(k_id){}
  TrioCombParams(){}
};
...
void SearchSingleRankActions(std::vector<Action>* actions, absl::Span<const int> hand, int prev_action);
...
void SearchChainOnlyActions(std::vector<Action>* actions, absl::Span<const int> hand, int prev_action);
...
void SearchSingleTrioCombActions(std::vector<Action>* actions, absl::Span<const int> hand, int prev_action);
...
void SearchAirplaneCombActions(std::vector<Action>* actions, absl::Span<const int> hand, int prev_action);
