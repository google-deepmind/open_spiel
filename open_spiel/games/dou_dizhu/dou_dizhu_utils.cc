...
int CardToRank(int card){
  if(card == kNumCards - 2 || card == kNumCards - 1) return card-kNumCards+kNumRanks;
  return card % (kNumRanks - 2);
}


std::string RankString(int rank){
  if(rank < kNumRanks - 2) return std::string(1, kRankChar[rank]);
  else if(rank == kNumRanks - 2) return "(BWJ)";
  else if(rank == kNumRanks - 1) return "(CJ)";
...
std::string FormatSingleHand(const std::array<int, kNumRanks>& hand){
  ...
  for (int rank = 0; rank < kNumRanks; ++rank){
    for (int i = 0; i < hand[rank]; ++i) hand_format += RankString(rank);
  }
  return hand_format;
}
...
std::string FormatAirplaneCombHand(int action){
  ...
  for(int rank = params.chain_head; rank < params.chain_head + params.chain_length; ++rank){
    for(int i = 0; i < 3; ++i) airplane_comb_str += RankString(rank);
  }
  airplane_comb_str += '-';
  // kickers
  for(int rank = 0; rank < kNumRanks; ++rank){
    if(rank >= params.chain_head && rank < params.chain_head + params.chain_length) continue;
    if(!hand[rank]) continue;
    for(int i = 0; i < hand[rank]; ++i) airplane_comb_str += RankString(rank);
  }
...
int GetNumCardsPerRank(int action){
  ...
  if(action >= kPlayActionBase && action < kPairActionBase) {
    ...
    num_cards = 2;
        ...
        (action >= kAirplaneActionBase && action < kAirplaneWithSoloActionBase)) {
    num_cards = 3;
  } else if(action >= kBombActionBase && action < kRocketActionBase) {
    num_cards = 4;
  } else SpielFatalError("Invalid action ID");
...
int GetSingleRankActionBase(int num_cards_same_rank=1){
  ...
  case 4:
    ...
    break;
  default:
    SpielFatalError("The number of cards of the same rank is wrong (single rank).");
  }
  return action_base;
}
...
SingleRankHandParams GetSingleRankHandParams(int action){
  int num_cards = GetNumCardsPerRank(action);
  int action_base = GetSingleRankActionBase(num_cards);
  SPIEL_CHECK_GE(action, action_base);
  int rank = action - action_base;
  return SingleRankHandParams(rank, num_cards);
}
...
int SingleRankHandToActionId(std::array<int, kNumRanks>& hand){
  ...
  for (int rank = 0; rank < kNumRanks; ++rank){
    if(hand[rank] != 0) {the_rank = rank; counter++;}
  }
  SPIEL_CHECK_EQ(counter, 1);
  int num_cards_same_rank = hand[the_rank];
...
void SearchSingleRankActions(std::vector<Action>& actions, std::array<int, kNumRanks>& hand, int prev_action = kInvalidAction){
  ...
  for(int rank = start_rank; rank < kNumRanks; ++rank){
    ...
    if(rank == kNumRanks - 2 || rank == kNumRanks - 1) SPIEL_CHECK_LE(hand[rank], 1);
...
ChainOnlyHandParams GetChainOnlyHandParams(int action){
  int num_cards_same_rank = GetNumCardsPerRank(action);
  int action_base = GetChainOnlyActionBase(num_cards_same_rank);
  int min_length = GetChainOnlyMinLength(num_cards_same_rank);
  SPIEL_CHECK_GE(action, action_base);
...
std::array<int, kNumRanks> ChainOnlyHand(int action){
  ...
  for(int i = 0; i < params.chain_length; ++i) hand[params.chain_head+i] = params.num_cards_per_rank;
  return hand;
}


int ChainOnlyHandToActionId(std::array<int, kNumRanks>& hand){
  int chain_head = -1, chain_length = 0, chain_counter = 0, num_cards_same_rank = 0;
  ...
  for (int rank = 0; rank < kNumRanks - 3; ++rank){
    if(hand[rank] == 0) {chain_stopped=true;}
    else{
      if(chain_stopped){
        chain_head = rank;
      ...
      else if(hand[rank] != num_cards_same_rank) SpielFatalError("Invalid pattern");
      else chain_length++;
    }
  }
  ...
  if(chain_length < min_length) 
    SpielFatalError(absl::StrFormat("The length of chain should be at least %d", min_length));
...
void SearchChainOnlyActions(std::vector<Action>& actions, std::array<int, kNumRanks>& hand, int prev_action = kInvalidAction){
  ...
  for(int chain_head=start_rank; chain_head < kNumRanks-4; ++chain_head){
    ...
    for(int chain_length = 2; chain_head + chain_length - 1 < kNumRanks - 3; ++chain_length){
      ...
      } else for(int n = 1; n <= num_cards; ++n) all_nums.push_back(n);
      for(auto n: all_nums){
        int min_length = GetChainOnlyMinLength(n);
        if(chain_length >= min_length){
...
int GetTrioCombActionBase(int action){
  ...
  } else if(kTrioWithPairActionBase <= action && action < kAirplaneActionBase) {
    action_base = kTrioWithPairActionBase;
  } else if(kAirplaneWithSoloActionBase <= action && action < kAirplaneWithPairActionBase){
    action_base = kAirplaneWithSoloActionBase;
  } else if(kAirplaneWithPairActionBase <= action && action < kBombActionBase){
    action_base = kAirplaneWithPairActionBase;
  } else SpielFatalError("Invalid action Ids");
  return action_base;
}
...
TrioCombParams GetAirplaneCombParams(int action){
  ...
  if(kAirplaneWithSoloActionBase <= action && action < kAirplaneWithPairActionBase){
    kicker_type = kSolo;
  ...
  for(chain_length = start_length; chain_length <= end_length; ++chain_length){
    int start_base = GetActionBaseFunc(chain_length);
    int next_base = chain_length == end_length? end_base: GetActionBaseFunc(chain_length+1);
  ...
  int hand_id = (action - action_base);
  int num_kickers = GetKickersNumFunc(chain_length);
  int chain_head = hand_id / num_kickers;
  int kicker_steps = hand_id % num_kickers;
...
std::array<int, kNumRanks> SingleTrioCombHand(int action){
  ...
  int kicker_steps = params.kicker_id;
...
int SingleTrioCombHandToActionId(std::array<int, kNumRanks> hand){
  ...
  for(int rank = 0; rank < kNumRanks; ++rank){
    if(hand[rank] == 3) {trio_counter++; trio_rank = rank;}
    else if(hand[rank] == 1 || hand[rank] == 2) {kicker_counter++; kicker_rank=rank;}
    else if(hand[rank] == 4) SpielFatalError("There cannot be a bomb");
  }
...
void SearchSingleTrioCombActions(std::vector<Action>& actions, std::array<int, kNumRanks>& hand, int prev_action = kInvalidAction){
...
// a dfs backtrack algorithm to compute action ids / hands for airplane combinations
// if target_count = -1, then the goal of this algorithm is to find the kicker_id
// of ans_hand, stored in count reference
// otherwise, the goal is to find a hand whose kicker_id is target_count
...
bool dfs_airplane_kicker(int chain_length, int depth,
      ...
      std::array<int, kNumRanks>& used_rank, std::array<int, kNumRanks>& ans_hand, 
      KickerType kicker_type){
    
  if(chain_length == depth){
    if(target_count == -1){
      bool found = true;
      for(int rank = 0; rank < kNumRanks; ++ rank) found = found & (used_rank[rank] == ans_hand[rank]);
      if(found) return true;
    }
    else if(target_count == count){
      for(int rank = 0; rank < kNumRanks; ++rank) ans_hand[rank] = used_rank[rank];
      return true;
    }
    count++;
  }else{
...
std::array<int, kNumRanks> AirplaneCombHand(int action){
  ...
  bool found = dfs_airplane_kicker(params.chain_length, 0, kicker_steps, 
        count, kNumRanks-1, used_rank, hand, params.kicker_type);
...
int AirplaneCombHandToActionId(std::array<int, kNumRanks> hand, 
  ...
  int chain_length = 0;
  bool chain_begined = false;
  std::vector<int> kickers;
  for(int rank = 0; rank < kNumRanks; ++rank){
    SPIEL_CHECK_LT(hand[rank], kNumSuits);
    if(!hand[rank]) continue;
    if(!chain_begined && rank != chain_head) {
      if(kicker_type == kSolo) 
        for(int i = 0; i < hand[rank]; ++i) kickers.push_back(rank);
      else{
        SPIEL_CHECK_EQ(hand[rank], 2); kickers.push_back(rank);
      }
    }else if(rank == chain_head) {
      SPIEL_CHECK_EQ(hand[rank], 3);
      chain_begined = true; chain_length++;
    } else if(chain_begined && hand[rank] == 3) chain_length++;
    else if(chain_begined && hand[rank] != 3){
      chain_begined = false;
      if(kicker_type == kSolo)
        for(int i = 0; i < hand[rank]; ++i) kickers.push_back(rank);
      else{
  ...
  // handle case where 333444555666 happened
  ...
  if(chain_head + chain_length - 1 >= kNumRanks - 3) SpielFatalError("2s, Joker cannot be in a chain");
  int action_base;
  if(kicker_type == kSolo)
    action_base = GetAirplaneSoloActionBase(chain_length) + chain_head * GetNumKickersAirplaneSoloComb(chain_length); 
  else action_base = GetAirplanePairActionBase(chain_length) + chain_head * GetNumKickersAirplanePairComb(chain_length);

  int count = 0;
  std::array<int, kNumRanks> used_rank{};
  for(int i = 0; i < chain_length; ++i) used_rank[chain_head+i] = 3;
  bool found = dfs_airplane_kicker(chain_length, 0, -1, count, kNumRanks-1, used_rank, hand, kicker_type);
  SPIEL_CHECK_TRUE(found);
...
void dfs_add_all_airplane_kickers(int chain_head, int chain_length, int depth, int max_search_rank,
  ...
  if(chain_length == depth){
    ...
    for(int rank = 0; rank <= max_search_rank; ++rank){
      ...
      dfs_add_all_airplane_kickers(chain_head, chain_length, depth+1, rank, used_rank, ans_hand, action_ids, kicker_type);
      used_rank[rank]-= num_use_cards;
    }
  }
}

void SearchAirplaneCombActions(std::vector<Action>& actions, std::array<int, kNumRanks>& hand, int prev_action = kInvalidAction){
  TrioCombParams prev_action_params;
  int start_rank;
  if(prev_action == kInvalidAction) start_rank = 0;
  else{
  ...
  for(int chain_head=start_rank; chain_head < kNumRanks-4; ++chain_head){
    ...
    for(int chain_length = 2; chain_head + chain_length - 1 < kNumRanks - 3; ++chain_length){
      ...
      } else {all_kicker_types.push_back(kSolo); all_kicker_types.push_back(kPair);}
      for(auto kicker_type: all_kicker_types){
        std::array<int, kNumRanks> used_hand{};
        for(int i = 0; i < chain_length; ++i) used_hand[chain_head+i] = 3;
        dfs_add_all_airplane_kickers(chain_head, chain_length, 0, kNumRanks-1, used_hand, hand, actions, kicker_type);
      }
    }
  }
}
...
std::array<int, kNumRanks> ActionToHand(int action){
  ...
  } else if(action >= kTrioWithSoloActionBase && action < kAirplaneActionBase){
    hand = SingleTrioCombHand(action);
  } else if(action >= kAirplaneWithSoloActionBase && action < kBombActionBase){
    hand = AirplaneCombHand(action);
  } else if(action == kRocketActionBase){
    hand[kNumRanks - 1] = hand[kNumRanks - 2] = 1;
  } else {SpielFatalError("Non valid Action Ids");}
  return hand;
}
...
void SearchForLegalActions(std::vector<Action>& legal_actions, std::array<int, kNumRanks>& hand, int prev_action){
  ...
  } else if (prev_action >= kBombActionBase && prev_action < kRocketActionBase){
    // if previous action is a bomb, then only higher bomb or rocket can be played
    SearchSingleRankActions(legal_actions, hand, prev_action);
  } else {
    // check for bombs
  ...
  if((prev_action >= kPlayActionBase && prev_action < kSoloChainActionBase) || 
     (prev_action >= kPairActionBase && prev_action < kPairChainActionBase) || 
     (prev_action >= kTrioActionBase && prev_action < kTrioWithSoloActionBase)){
       SearchSingleRankActions(legal_actions, hand, prev_action);
  } else if((prev_action >= kSoloChainActionBase && prev_action < kPairActionBase) ||
  (prev_action >= kPairChainActionBase && prev_action < kTrioActionBase) ||
  (prev_action >= kAirplaneActionBase && prev_action < kAirplaneWithSoloActionBase)){
    SearchChainOnlyActions(legal_actions, hand, prev_action);
  } else if(prev_action >= kTrioWithSoloActionBase && prev_action < kAirplaneActionBase){
    SearchSingleTrioCombActions(legal_actions, hand, prev_action);
  } else if(prev_action >= kAirplaneWithSoloActionBase && prev_action < kBombActionBase){
    SearchAirplaneCombActions(legal_actions, hand, prev_action);
  } else if(prev_action == kRocketActionBase){}
    else {SpielFatalError("Previous actions invalid");}
  }
}
