...
GoFishState::GoFishState(std::shared_ptr<const Game> game,
  ...
  : State(game),
    ...
    most_books_wins_(static_cast<const GoFishGame*>(
      game.get())->MostBooksWins()),
    end_on_first_out_(static_cast<const GoFishGame*>(
      game.get())->EndOnFirstOut()),
    ask_after_empty_draw_(static_cast<const GoFishGame*>(
      game.get())->AskAfterEmptyDraw()),
...
std::vector<double> GoFishState::Returns() const {
  ...
  if (!most_books_wins_) {
    ...
    int max_score = *std::max_element(player_books_.begin(),
      player_books_.end());
    int num_winners = std::count(player_books_.begin(), player_books_.end(),
       max_score);
...
void GoFishState::ObservationTensor(Player player,
  ...
  for (int pid = 0; pid < num_players_; ++pid) {
    ...
    for (int rank = 0; rank < ranks_; ++rank) {
      ...
      values[offset++] = 1.0f *
        drawn_since_was_asked_[pid][rank]/ (ranks_ * suits_);
...
std::string GoFishState::ObservationString(Player player) const {
  ...
  for (int rank = 0; rank < ranks_; ++rank) {
    if (player_cards_[player][rank] > 0) {
      absl::StrAppend(&result, RankString(rank),
        player_cards_[player][rank], " ");
    }
  }
  absl::StrAppend(&result, "\n");
  for (int pid = 0; pid < num_players_; ++pid) {
