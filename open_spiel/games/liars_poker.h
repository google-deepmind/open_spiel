...
#ifndef OPEN_SPIEL_GAMES_LIARS_POKER_H_
#define OPEN_SPIEL_GAMES_LIARS_POKER_H_
...
class LiarsPokerState : public State {
 public:
  explicit LiarsPokerState(std::shared_ptr<const Game> game, int total_num_dice,
                          int max_dice_per_player,
                          const std::vector<int>& num_dice);
  LiarsPokerState(const LiarsPokerState&) = default;
  ...
  int last_bid() const {
    for (auto rit=bidseq_.rbegin(); rit!=bidseq_.rend(); ++rit) {
      if (*rit < total_num_dice_ * dice_sides()) {
	return *rit;
      }
    }
    return -1;
 ...
 private:
  void ResolveWinner();
  bool isCALLPossible() const;
  bool isCOUNTSPossible() const;
  int CALL;
  int COUNTS;
...
class LiarsPokerGame : public Game {
 public:
  explicit LiarsPokerGame(const GameParameters& params, GameType game_type);
...
class ImperfectRecallLiarsPokerState : public LiarsPokerState {
 public:
  ImperfectRecallLiarsPokerState(std::shared_ptr<const Game> game,
                                int total_num_dice,
                                int max_dice_per_player,
                                const std::vector<int>& num_dice)
      : LiarsPokerState(game, total_num_dice, max_dice_per_player, num_dice) {}
  std::string InformationStateString(Player player) const override;
  std::unique_ptr<State> Clone() const override {
    return std::unique_ptr<State>(new ImperfectRecallLiarsPokerState(*this));
  }
};

class ImperfectRecallLiarsPokerGame : public LiarsPokerGame {
 public:
  explicit ImperfectRecallLiarsPokerGame(const GameParameters& params);
  std::unique_ptr<State> NewInitialState() const override;
...
#endif  // OPEN_SPIEL_GAMES_LIARS_POKER_H_
