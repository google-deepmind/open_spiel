...
void TerminalReturnsTests() {
  std::shared_ptr<const Game> clobber2x2 = LoadGame("clobber(rows=2,columns=2)");
  std::shared_ptr<const Game> clobber4x4 = LoadGame("clobber(rows=4,columns=4)");
  std::shared_ptr<const Game> clobber5x6 = LoadGame("clobber(rows=5,columns=6)");
  ...
  ClobberState ongoing_state(clobber5x6, 5, 6, "0ox..ox..oxoxox..ox..oxoxoxoxox");
  SPIEL_CHECK_EQ(ongoing_state.IsTerminal(), false);
}

void ObservationTensorTests() {
  std::shared_ptr<const Game> clobber8x8 = LoadGame("clobber(rows=8,columns=8)");
