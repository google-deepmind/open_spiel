...
#ifndef OPEN_SPIEL_GAMES_VAQUERITOS_H_
#define OPEN_SPIEL_GAMES_VAQUERITOS_H_
...
class VaqueritosState : public State {
 public:
  VaqueritosState(std::shared_ptr<const Game> game);

  VaqueritosState(const VaqueritosState&) = default;
  VaqueritosState& operator=(const VaqueritosState&) = default;
...
// Game object.
class VaqueritosGame : public Game {
 public:
  explicit VaqueritosGame(const GameParameters& params);
  int NumDistinctActions() const override { return kNumCells; }
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new VaqueritosState(shared_from_this()));
  }
...
#endif  // OPEN_SPIEL_GAMES_VAQUERITOS_H_
