...
PenniesState::PenniesState(std::shared_ptr<const Game> game)
    : State(game) {}
...
bool PenniesState::IsTerminal() const {
    return actionA_ != kNoAction && actionB_ != kNoAction && infoset_ != kNoInfoset;
}

std::vector<double> PenniesState::Returns() const {
    if (!IsTerminal()) return {0., 0.};
    const int matching = actionA_ == actionB_ ? 1 : -1;
...
std::unique_ptr<State> PenniesState::Clone() const {
    return std::unique_ptr<State>(new PenniesState(*this));
}
...
std::unique_ptr<State> PenniesGame::NewInitialState() const {
    return std::unique_ptr<State>(new PenniesState(shared_from_this()));
}
