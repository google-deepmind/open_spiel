...
std::string DouDizhuState::ActionToString(Player player, Action action) const {
    if (player == kChancePlayerId) {
        if (action < kDealingActionBase) {
            ...
            return absl::StrFormat("Deal %s",
                                   CardString(action - kDealingActionBase));
        } else {
            SpielFatalError(
...
std::string DouDizhuState::FormatDeal() const {
    ...
    if (IsTerminal()) {
        // Include all cards in the terminal state to make reviewing the deal
        // easier
...
std::string DouDizhuState::ObservationString(Player player) const {
    ...
    absl::StrAppend(
        &rv, absl::StrFormat("face up card rank: %d", card_rank_face_up_));
...
void DouDizhuState::WriteObservationTensor(Player player,
    ...
    for (int i = 0; i < kNumRanks; ++i) {
        ...
        values_iterator[played_deck_base + i * (kNumSuits + 1) +
                        played_deck_[i]] = 1;
    }
    ...
    if (first_player_ != kInvalidPlayer) {
        ...
        values_iterator[start_player_base + kNumPlayers + card_rank_face_up_] =
            1;
    }
}
...
std::vector<Action> DouDizhuState::DealLegalActions() const {
    ...
    if (card_face_up_position_ == -1) {
        ...
        for (int i = 0; i < kNumCards; ++i) {
            if (dealer_deck_[i])
                legal_actions.push_back(i + kDealingActionBase);
        }
    }
    ...
    if (card_face_up_position_ == -1) {
        for (int i = 0; i < kDealingActionBase; ++i)
            outcomes.emplace_back(
                i, 1.0 / static_cast<double>(kDealingActionBase));
    } else {
        for (int card = 0; card < kNumCards; ++card)
            if (dealer_deck_[card])
                outcomes.emplace_back(
                    card + kDealingActionBase,
