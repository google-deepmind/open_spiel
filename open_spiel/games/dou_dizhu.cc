...
#include "open_spiel/games/dou_dizhu.h"
...
std::string DouDizhuState::ActionToString(Player player, Action action) const{
  ...
  } else if(action == kPass) return "Pass";
...
std::string DouDizhuState::FormatAuction() const{
  ...
  for(int i = kNumCards - kNumCardsLeftOver + 1; i < history_.size() - num_played_; ++i){
    absl::StrAppend(&rv, absl::StrFormat("Player %d played %s\n", 
       history_[i].player, ActionToString(history_[i].player, history_[i].action)));
  }
  return rv;
}
...
std::string DouDizhuState::FormatResult() const{
  ...
  for(int player = 0; player < kNumPlayers; ++ player){
    absl::StrAppend(&rv, absl::StrFormat("Player %d got %f\n", player, returns_[player]));
  }
  return rv;
}
...
std::array<std::string, kNumRanks> FormatHand(
  ...
  if (deal[player][kNumRanks-2]) absl::StrAppend(&cards[kNumRanks-2], "(BWJ)");
  else if(mark_voids) absl::StrAppend(&cards[kNumRanks-2], "none");

  if (deal[player][kNumRanks-1]) absl::StrAppend(&cards[kNumRanks-1], "(CJ)");
  else if(mark_voids) absl::StrAppend(&cards[kNumRanks-1], "none");
...
std::array<std::array<int, kNumRanks>, kNumPlayers> DouDizhuState::OriginalDeal()
    const {
  ...
  for (int i = 1; i < kNumCards - kNumCardsLeftOver + 1; ++i)
    deal[((i - 1 + first_player_) % kNumPlayers)][CardToRank(history_[i].action)]++ ;
...
std::string DouDizhuState::ObservationString(Player player) const{
  ...
  absl::StrAppend(&rv, absl::StrFormat("My position from Dizhu: %d", (player - dizhu_ + kNumPlayers) % kNumPlayers));
  return rv;
}
...
void DouDizhuState::ApplyBiddingAction(int action) {
  ...
  if (action == kPass){
    if (num_passes_ == kNumPlayers) phase_ = Phase::kGameOver;
    else if (num_passes_ == kNumPlayers - 1 && winning_bid_ > 0) has_winner = true;
  } else{
  ...
  if (has_winner){
    for (int i = 0; i < kNumCardsLeftOver; ++i) holds_[dizhu_][cards_left_over_[i]]++;
    phase_ = Phase::kPlay;
...
void DouDizhuState::ScoreUp() {
  ...
  is_spring |= ((!players_hands_played[(dizhu_ + 1) % 3]) && (!players_hands_played[(dizhu_ + 2) % 3]));
