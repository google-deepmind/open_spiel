...
constexpr std::array<int, kNumDirections> kDirRowOffsets = {
    {-1, 0, 1, 0}};
constexpr std::array<int, kNumDirections> kDirColumnOffsets = {
    {0, 1, 0, -1}};
...
ClobberState::ClobberState(std::shared_ptr<const Game> game,
...
void ClobberState::DoApplyAction(Action action_id) {
  std::vector<int> values(3, -1);
  UnrankActionMixedBase(action_id,
                        {rows_, columns_, kNumDirections},
                        &values);

  int start_row = values[0];
  int start_column = values[1];
  int direction = values[2];
  int end_row = start_row + kDirRowOffsets[direction];
  int end_column = start_column + kDirColumnOffsets[direction];
...
std::string ClobberState::ActionToString(Player player,
  ...
  absl::StrAppend(&action_string, ColumnLabel(start_column));
  absl::StrAppend(&action_string, RowLabel(rows_, start_row));
  absl::StrAppend(&action_string, ColumnLabel(end_column));
  absl::StrAppend(&action_string, RowLabel(rows_, end_row));
...
std::vector<Action> ClobberState::LegalActions() const {
  ...
  if (IsTerminal()) { return move_list; }
...
bool ClobberState::IsTerminal() const {
...
void ClobberState::ObservationTensor(Player player,
  ...
  TensorView<3> view(values, {kNumPlayers + 1, rows_, columns_}, true);
  ...
  for (int row = 0; row < rows_; row++) {
    for (int column = 0; column < columns_; column++) {
      // NOTE: 'current_player_' is used instead of 'player' as the
      //       Python version does not always call this function with
      //       the Player argument and it defaults to 0.
      int plane = ObservationPlane(BoardAt(row, column), current_player_);
      view[{plane, row, column}] = 1.0;
    }
  }
}

void ClobberState::UndoAction(Player player, Action action_id) {
  std::vector<int> values(3, -1);
  UnrankActionMixedBase(action_id,
                        {rows_, columns_, kNumDirections},
                        &values);
