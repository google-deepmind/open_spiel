...
void CaptureTheFlagState::ResolveMove(Player player, int move) {
  ...
  if (flag_holder_[opponent_flag] == -1 &&
      new_r == opp_flag_home_r && new_c == opp_flag_home_c) {
  ...
  const bool own_flag_home = (flag_holder_[player] == -1 &&
                              flag_row_[player] == own_base_r &&
...
void CaptureTheFlagState::DoApplyAction(Action action_id) {
  ...
  if (action_id == kChanceInit0Action) {
    ...
    if (winner_ == -1) ResolveMove(1, moves_[1]);
  } else {
    ResolveMove(1, moves_[1]);
    if (winner_ == -1) ResolveMove(0, moves_[0]);
  }
  if (winner_ == -1) ResolveTags();
  total_moves_ += 1;
  cur_player_ = kSimultaneousPlayerId;
}
