...
const GameType kGameType{/*short_name=*/"crazy_eights",
...
CrazyEightsGame::CrazyEightsGame(const GameParameters& params): 
  Game(kGameType, params), num_players_(ParameterValue<int>("players")), max_draw_cards_(ParameterValue<int>("max_draw_cards"))
  , use_special_cards_(ParameterValue<bool>("use_special_cards")), reshuffle_(ParameterValue<bool>("reshuffle")) {}

CrazyEightsState::CrazyEightsState(std::shared_ptr<const Game> game, int num_players, int max_draw_cards, bool use_special_cards, bool reshuffle): 
State(game), num_players_(num_players), max_draw_cards_(max_draw_cards), use_special_cards_(use_special_cards), reshuffle_(reshuffle){
  num_initial_cards_ = num_players == 2? 7: 5;
...
std::string CrazyEightsState::ActionToString(Player player, Action action) const{
  std::string str;
  if(action < kDraw){
    ...
    absl::StrAppend(&str, absl::StrFormat("Nominate suit %c", kSuitChar[action-kNominateSuitActionBase]));
  } else if(action < kDecideDealerActionBase + num_players_){
    absl::StrAppend(&str, absl::StrFormat("Decide Player %d to be the dealer", action-kDecideDealerActionBase));
...
std::vector<std::string> CrazyEightsState::FormatHand(Player player) const{
  std::vector<std::string> hand_str(kNumSuits, std::string(num_decks_ * kNumRanks, ' '));
...
std::string CrazyEightsState::FormatAllHands() const{
  ...
  for(int player = 0; player < num_players_; ++player){
    ...
    if(player != num_players_ - 1){
      absl::StrAppend(&player_str, std::string(kLongWidth-player_str.length(), ' '));
    }else {
  ...
  for(int suit = 0; suit < kNumSuits; ++suit){
    ...
    for(int player = 0; player < num_players_; ++player){
      ...
      absl::StrAppend(&player_row, absl::StrFormat("Suit %c: %s", kSuitChar[suit], all_hands[player][suit]));
      SPIEL_CHECK_GE(kLongWidth, player_row.length());
      if(player != num_players_ - 1){
        absl::StrAppend(&player_row, std::string(kLongWidth-player_row.length(), ' '));
      } else{
...
std::string CrazyEightsState::ToString() const{
  ...
  for(int i = 0; i < history_.size(); ++i){
    if(i == 0) absl::StrAppend(&str, absl::StrFormat("Player %d becomes the dealer\n", dealer_));
    else if(i <= num_players_ * num_initial_cards_){
      int player = (dealer_ + i) % num_players_;
      absl::StrAppend(&str, absl::StrFormat("Player %d is dealt %s\n", player, GetCardStr(history_[i].action)));
    } else{
      if(history_[i].player == kChancePlayerId) {
        absl::StrAppend(&str, absl::StrFormat("Player %d draws %s\n", playing_player, GetCardStr(history_[i].action)));
      }
        ...
        } else if(history_[i].action >= kNominateSuitActionBase && history_[i].action < kDecideDealerActionBase){
          int suit = history_[i].action - kNominateSuitActionBase;
          absl::StrAppend(&str, absl::StrFormat("Player %d nominates suit %c\n", playing_player, kSuitChar[suit]));
        } else{
          SPIEL_CHECK_GE(history_[i].action, 0);
          SPIEL_CHECK_LT(history_[i].action, kNumCards);
          absl::StrAppend(&str, absl::StrFormat("Player %d plays %s\n", playing_player, GetCardStr(history_[i].action)));
        }
      } else{
  ...
  if(last_card_ != kInvalidAction) {
    absl::StrAppend(&str, absl::StrFormat("Last card: %s\n", GetCardStr(last_card_)));
    absl::StrAppend(&str, absl::StrFormat("Last suit: %c\n", kSuitChar[last_suit_]));
  }
...
std::string CrazyEightsState::ObservationString(Player player) const{
  ...
  for(int suit = 0; suit < kNumSuits; ++suit){
    absl::StrAppend(&str, absl::StrFormat("Suit %c: %s\n", kSuitChar[suit], hands[suit]));
  }
  absl::StrAppend(&str, absl::StrFormat("Previous card: %s\n", GetCardStr(last_card_)));
  absl::StrAppend(&str, absl::StrFormat("Previous suit: %c\n", kSuitChar[last_suit_]));
...
std::vector<std::pair<Action, double>> CrazyEightsState::ChanceOutcomes() const {
  std::vector<std::pair<Action, double>> outcomes;

  if(!history_.size()){
    for(int player = 0; player < num_players_; ++player){
      outcomes.emplace_back(player+kDecideDealerActionBase, 1.0/num_players_);
    }
  }else{
    ...
    for(int card = 0; card < kNumCards; ++card){
      if(dealer_deck_[card]){
        outcomes.emplace_back(card, static_cast<double>(dealer_deck_[card])/num_cards_remaining);
      }
    }
  }
  return outcomes;
...
void CrazyEightsState::ApplyDealAction(int action){
  ...
  // redraw=true if we are examining the first card turned face up after the initial
  // dealing round, which cannot be Eights
  if(redraw_){
...
void SearchLegalCards(std::vector<Action>* legal_actions, 
    const std::vector<int>& hand, int last_rank, int last_suit){
  
  for(int card = 0; card < kNumCards; ++card){
    if(hand[card] == 0) continue;
    Suit suit = GetSuit(card);
    int rank = GetRank(card);
    if(rank == kEightRank) legal_actions->push_back(card);
    else if(last_suit == suit || last_rank == rank) legal_actions->push_back(card);
  }
  return;
}
  ...
  if(num_draws_left_){
    ...
    if(!start_draw_twos_){
      for(int suit = kClubs; suit <= kSpades; ++suit){
        ...
        if(hands_[current_player_][duo_card]) legal_actions.push_back(duo_card);
      }
    }
  } else{
...
bool CrazyEightsState::AfterPlayCard(int action){
...
void CrazyEightsState::ApplyPlayAction(int action){
  if(action == kPass){
    if(!num_cards_left_) num_passes_++;
    else num_passes_ = 0;
  ...
  if(action == kDraw){
    ...
    if(use_special_cards_){
      if(rank == kSkipRank){
        current_player_ = (current_player_ + 2 * direction_ + num_players_) % num_players_;
        return;
      }
...
Player CrazyEightsState::CurrentPlayer() const {
  ...
  else if(phase_ == Phase::kGameOver) return kTerminalPlayerId;
  else return current_player_;
}
...
void CrazyEightsState::ScoreUp(){
  for(int player = 0; player < num_players_; ++player){
    for(int card = 0; card < kNumCards; ++card){
      ...
      int rank = GetRank(card);
      if(rank == kEightRank) returns_[player] -= 50 * hands_[player][card];
      else if(rank >= 9) returns_[player] -= 10 * hands_[player][card];
      else returns_[player] -= (card + 2) *  hands_[player][card];
    }
  }
}
