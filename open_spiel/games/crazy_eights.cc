...
std::string CrazyEightsState::ToString() const {
    ...
    for (int i = 0; i < history_.size(); ++i) {
        if (i == 0) {
            absl::StrAppend(
                &str,
                absl::StrFormat("Player %d becomes the dealer\n", dealer_));
        } else if (i <= num_players_ * num_initial_cards_) {
            int player = (dealer_ + i) % num_players_;
            absl::StrAppend(&str,
                            absl::StrFormat("Player %d is dealt %s\n", player,
            ...
            if (history_[i].player == kChancePlayerId) {
                ...
                if (history_[i].action == kDraw) {
                    absl::StrAppend(
                        &str, absl::StrFormat("Player %d starts drawing\n",
                                              playing_player));
                    ...
                    absl::StrAppend(
                        &str,
                        absl::StrFormat("Player %d plays %s\n", playing_player,
                                        GetCardStr(history_[i].action)));
                }
            } else {
                ...
                for (int player = 0; player < num_players_; ++player) {
                    absl::StrAppend(&str,
                                    absl::StrFormat("Player %d gets score %f\n",
    ...
    if (last_card_ != kInvalidAction) {
        absl::StrAppend(
            &str, absl::StrFormat("Last card: %s\n", GetCardStr(last_card_)));
        absl::StrAppend(
            &str, absl::StrFormat("Last suit: %c\n", kSuitChar[last_suit_]));
    }
...
void CrazyEightsState::WriteObservationTensor(Player player,
    ...
    for (int i = 1; i <= num_players_ - 1; ++i) {
        ...
        values[tmp_base + (i - 1) * (num_decks_ * kNumCards + 1) + num_cards] =
            1;
    }
    ...
    if (!history_.size()) {
        ...
        for (int card = 0; card < kNumCards; ++card) {
            if (dealer_deck_[card]) {
                outcomes.emplace_back(card,
                                      static_cast<double>(dealer_deck_[card]) /
                                          num_cards_remaining);
            }
        }
    }
        ...
        if (rank != kEightRank) {
            ...
            if (use_special_cards_) {
                if (rank == kSkipRank) {
                    ...
                    current_player_ =
                        (current_player_ - 1 + num_players_) % num_players_;
                    direction_ *= -1;
                    return;
...
void CrazyEightsState::ApplyPlayAction(int action) {
    ...
    if (action == kDraw) {
        ...
        if (use_special_cards_) {
            if (rank == kSkipRank) {
                current_player_ =
                    (current_player_ + 2 * direction_ + num_players_) %
                    num_players_;
                return;
            }
            if (rank == kReverseRank) {
                direction_ *= -1;
                current_player_ =
                    (current_player_ + direction_ + num_players_) %
                    num_players_;
                return;
            }
