...
#include "open_spiel/games/liars_poker.h"
...
const GameType kGameType{
    /*short_name=*/"liars_poker",
    /*long_name=*/"Liars Poker",
...
const GameType kImperfectRecallGameType{
    /*short_name=*/"liars_poker_ir",
    /*long_name=*/"Liars Poker with Imperfect Recall",
    GameType::Dynamics::kSequential,
    GameType::ChanceMode::kExplicitStochastic,
    GameType::Information::kImperfectInformation,
    GameType::Utility::kZeroSum,
...
std::shared_ptr<const Game> Factory(const GameParameters& params) {
  return std::shared_ptr<const Game>(new LiarsPokerGame(params, kGameType));
}

std::shared_ptr<const Game> ImperfectRecallFactory(
    const GameParameters& params) {
  return std::shared_ptr<const Game>(new ImperfectRecallLiarsPokerGame(params));
}
...
const LiarsPokerGame* UnwrapGame(const Game* game) {
  return down_cast<const LiarsPokerGame*>(game);
}
}  // namespace
...
LiarsPokerState::LiarsPokerState(std::shared_ptr<const Game> game,
  ...
  CALL = total_num_dice_ * dice_sides();
  COUNTS = CALL + 1;
}

std::string LiarsPokerState::ActionToString(Player player,
                                           Action action_id) const {
  if (player != kChancePlayerId) {
    if (action_id == CALL) {
      return "CALL";
    } else if (action_id == COUNTS) {
      return "COUNTS";
    } else {
...
int LiarsPokerState::CurrentPlayer() const {
  if (IsTerminal()) {
...
bool LiarsPokerState::isCALLPossible() const {
  // CALL is possible if COUNTS is not, and at least one bid has been played.
  if (bidseq_.size() < 1) return false;
  else if (isCOUNTSPossible()) return false;
  else return true;
}

bool LiarsPokerState::isCOUNTSPossible() const {
  // COUNTS is possible if other player has CALLed, or of this player has CALLed in their latest turn.
  int bid_len = bidseq_.size();
  if (bid_len < 2)
    return false;
  else if (bidseq_[bid_len-1] == CALL || bidseq_[bid_len-2] == CALL)
    return true;
  else
    return false;
}

void LiarsPokerState::ResolveWinner() {
...
const int LiarsPokerState::dice_sides() const {
  return UnwrapGame(game_.get())->dice_sides();
}

const BiddingRule LiarsPokerState::bidding_rule() const {
  return UnwrapGame(game_.get())->bidding_rule();
}

void LiarsPokerState::DoApplyAction(Action action) {
  if (IsChanceNode()) {
    ...
    auto action_mask = LegalActionsMask();
    ...
    if (action == CALL) {
      // This was the calling bid, game is not over yet. Opponent may bid higher or play COUNTS.
      // If opponent played higher, this player can play COUNTS and remain the calling_player at game end.
      ...
      // COUNTS was played. Game is now over. calling_player must be set already.
      bidseq_.push_back(action);
      ResolveWinner();
    }
    else {
...
std::vector<Action> LiarsPokerState::LegalActions() const {
  ...
  if (isCOUNTSPossible()) {
    actions.push_back(COUNTS);
  }

  return actions;
}

std::vector<std::pair<Action, double>> LiarsPokerState::ChanceOutcomes() const {
...
std::string LiarsPokerState::InformationStateString(Player player) const {
  ...
  for (int b = 0; b < bidseq_.size(); b++) {
    if (bidseq_[b] == CALL) {
      absl::StrAppend(&result, " CALL");
    } else if (bidseq_[b] == COUNTS) {
      absl::StrAppend(&result, " COUNTS");
    } else {
...
std::string LiarsPokerState::ToString() const {
  std::string result = "";
...
bool LiarsPokerState::IsTerminal() const { return winner_ != kInvalidPlayer; }

std::vector<double> LiarsPokerState::Returns() const {
...
void LiarsPokerState::InformationStateTensor(Player player,
  ...
  // One slot(bit) for CALL after each bid.
  // One slot(bit) for COUNTS (Necessary because observations and
  ...
  SPIEL_CHECK_EQ(values.size(), num_players_ +
                                    ...
                                    2 * (total_num_dice_ * dice_sides()) + 1);
  values[player] = 1;
  offset += num_players_;
  ...
  for (int b = 0; b < bidseq_.size(); b++) {
    ...
    SPIEL_CHECK_LE(bidseq_[b], total_num_dice_ * dice_sides() + 1);
    if (bidseq_[b] == CALL) {
      values[offset + 2 * bidseq_[b-1] + 1] = 1;
    } else if (bidseq_[b] == COUNTS) {
      values[offset + 2 * total_num_dice_ * dice_sides()] = 1;
    } else {
      values[offset + 2 * bidseq_[b]] = 1;
    }
  }
}

void LiarsPokerState::ObservationTensor(Player player,
                                       absl::Span<float> values) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
...
std::unique_ptr<State> LiarsPokerState::Clone() const {
  return std::unique_ptr<State>(new LiarsPokerState(*this));
}

std::pair<int, int> LiarsPokerState::UnrankBid(int bidnum) const {
...
LiarsPokerGame::LiarsPokerGame(const GameParameters& params, GameType game_type)
...
int LiarsPokerGame::NumDistinctActions() const {
  // A bet for each side and number of total dice, "CALL" action and "COUNTS" action.
  return total_num_dice_ * dice_sides_ + 2;
}

std::unique_ptr<State> LiarsPokerGame::NewInitialState() const {
  std::unique_ptr<LiarsPokerState> state(
      new LiarsPokerState(shared_from_this(),
...
int LiarsPokerGame::MaxChanceOutcomes() const { return dice_sides_; }

int LiarsPokerGame::MaxGameLength() const {
  // A bet for each side and number of total dice. 
  int length = total_num_dice_ * dice_sides_;
  // After every two bets, there can be a CALL. 
  length += length / 2 + 1;
  // Plus COUNTS.
  printf("Max game length set to %d\n", length + 1);
  return length + 1;
}
int LiarsPokerGame::MaxChanceNodesInHistory() const { return total_num_dice_; }

std::vector<int> LiarsPokerGame::InformationStateTensorShape() const {
  ...
  return {num_players_ + (max_dice_per_player_ * dice_sides_) +
          2 * (total_num_dice_ * dice_sides_) + 1};
}

std::vector<int> LiarsPokerGame::ObservationTensorShape() const {
  // One-hot encoding for the player number.
  // One-hot encoding for each die (max_dice_per_player_ * sides).
  // One slot(bit) for CALL after each bid.
  // One slot(bit) for COUNTS (Necessary because observations and
...
ImperfectRecallLiarsPokerGame::ImperfectRecallLiarsPokerGame(
    const GameParameters& params)
    : LiarsPokerGame(params, kImperfectRecallGameType),
      recall_length_(
          ParameterValue<int>("rollout_length", kDefaultRecallLength)) {}

std::unique_ptr<State> ImperfectRecallLiarsPokerGame::NewInitialState() const {
  return absl::make_unique<ImperfectRecallLiarsPokerState>(shared_from_this(),
      /*total_num_dice=*/total_num_dice(),
      /*max_dice_per_player=*/max_dice_per_player(),
      /*num_dice=*/num_dice());
}

std::string ImperfectRecallLiarsPokerState::InformationStateString(
    Player player) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
  const auto* parent_game = down_cast<const ImperfectRecallLiarsPokerGame*>(
      game_.get());
  ...
  for (int b = start_index; b < bidseq_.size(); b++) {
    if (bidseq_[b] == parent_game->total_num_dice() * dice_sides()) {
      absl::StrAppend(&result, " CALL");
    } else if (bidseq_[b] == parent_game->total_num_dice() * dice_sides() + 1) {
      absl::StrAppend(&result, " COUNTS");
    } else {
      const std::pair<int, int> bid = UnrankBid(bidseq_[b]);
      absl::StrAppend(&result, " ", bid.first, "-", bid.second);
