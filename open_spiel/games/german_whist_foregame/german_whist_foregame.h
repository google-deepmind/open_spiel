...
constexpr std::array<std::array<uint32_t, 33>, 33> BIN_COEFFS_LUT =
    BinCoeffsLUT<32>();

// Reimplementing bmi2 intrinsics with bit operations that will work on all
// platforms//
...
// containers of cards are 64 bits,with the least significant 52bits being the
// suits CDHS,with the least sig bit of each suit being the highest rank card//
// this container of masks is used to extract only the cards from a suit//
inline const std::array<uint64_t, 4> kSuitMasks = {
    bzhi_u64(~0, kNumRanks),
    bzhi_u64(~0, 2 * kNumRanks) ^ bzhi_u64(~0, kNumRanks),
    bzhi_u64(~0, 3 * kNumRanks) ^ bzhi_u64(~0, 2 * kNumRanks),
    bzhi_u64(~0, 4 * kNumRanks) ^ bzhi_u64(~0, 3 * kNumRanks)};
...
class GWhistFState : public State {
 public:
  ...
  std::pair<uint32_t, uint32_t> EndgameKey(int player_to_move) const;
