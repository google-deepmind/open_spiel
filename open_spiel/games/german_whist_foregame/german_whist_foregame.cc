...
vectorNa InitialiseTTable(
    int size, const std::vector<std::vector<uint32_t>>& bin_coeffs) {
  ...
  for (int i = 0; i < kNumSuits; ++i) {
    if (i != trump_) {
      ...
      suit_lengths.push_back(
          Triple{static_cast<char>(i),
                 static_cast<char>(popcnt_u64(kSuitMasks[i] & cards_in_play)),
...
std::unique_ptr<State> GWhistFState::ResampleFromInfostate(
  ...
  for (int i = 2 * kNumRanks; i < history_.size(); i += 4) {
    ...
    necessary_cards =
        (necessary_cards | static_cast<uint64_t>(1) << history_[i].action);
  }
  ...
  uint64_t recent_faceup_card =
      (static_cast<uint64_t>(1) << history_[recent_faceup].action);
  ...
  for (int i = 0; i < kNumSuits; ++i) {
    if (voids[i] != -1 &&
      ...
      for (int j = 0; j < voids[i]; ++j) {
        sufficient_cards =
            (sufficient_cards | static_cast<uint64_t>(1) << temp[j]);
      }
    }
  }
  ...
  for (int i = 0; i < popcnt_u64(hands_[opp]) - nec; ++i) {
    suff_hand = suff_hand | (static_cast<uint64_t>(1) << hand_vec[i]);
  }
