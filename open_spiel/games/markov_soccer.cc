...
MarkovSoccerState::MarkovSoccerState(std::shared_ptr<const Game> game, const Grid& grid)
...
std::string MarkovSoccerState::ActionToString(Player player,
  ...
  if (player == kChancePlayerId) {
    ...
    if (action_id == kChanceInit0Action) {
      ...
      return absl::StrCat("(ball dropped at location #",
...
std::vector<Action> MarkovSoccerState::LegalActions(Player player) const {
  ...
  if (IsChanceNode()) {
    if (total_moves_ == 0) {
      std::vector<Action> outcomes(grid_.ball_start_points.size(), kInvalidAction);
      for (int i = 0; i < grid_.ball_start_points.size(); ++i) {
        outcomes[i] = kNumInitiativeChanceOutcomes + i;
      }
      return outcomes;
    } else {
      return {ToAction(ChanceOutcome::kChanceInit0), ToAction(ChanceOutcome::kChanceInit1)};
    }
  } else {
...
std::vector<std::pair<Action, double>> MarkovSoccerState::ChanceOutcomes()
  ...
  SPIEL_CHECK_TRUE(IsChanceNode());
  if (total_moves_ == 0) {
    std::vector<std::pair<Action, double>> outcomes(grid_.ball_start_points.size(),
                                                    {kInvalidAction, -1.0});
    ...
    return {std::pair<Action, double>(ToAction(ChanceOutcome::kChanceInit0), 0.5),
