...
void AddPieceTypePlane(Color color, PieceType piece_type,
  ...
  for (int8_t y = 0; y < kBoardSize; ++y) {
    for (int8_t x = 0; x < kBoardSize; ++x) {
      ...
      *value_it++ =
          (piece_on_board.color == color and piece_on_board.type == piece_type
               ? 1.0
               : 0.0);
    }
  }
...
Action MoveToAction(const Move& move) {
  ...
  if (move.promotion_type != PieceType::kEmpty and
    ...
    SPIEL_CHECK_TRUE(player_move.from.y == kBoardSize - 2 and
    ...
    SPIEL_CHECK_TRUE(kNumUnderPromotions <= destination_action_index and
...
Move ActionToMove(const Action& action, const ChessBoard& board) {
  ...
  if (not is_under_promotion and piece.type == PieceType::kPawn and
      ReflectRank(color, kBoardSize, from_square.y) == kBoardSize - 2 and
...
std::unique_ptr<State> AntichessGame::DeserializeState(
  ...
  if (not absl::StartsWith(str, prefix)) {
