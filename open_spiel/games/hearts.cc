...
const GameType kGameType{/*short_name=*/"hearts",
                         /*long_name=*/"Hearts",
...
std::map<int, std::string> pass_dir_str = {{0, "No Pass"},
                                           {1, "Left"},
                                           {2, "Across"},
                                           {3, "Right"}};
...
HeartsState::HeartsState(std::shared_ptr<const Game> game,
                         bool pass_cards,
...
std::string HeartsState::ActionToString(Player player, Action action) const {
  if (history_.size() == 0) return pass_dir_str[action];
...
std::string HeartsState::ToString() const {
  ...
  if (passed_cards_[0].size() > 0) absl::StrAppend(&rv, FormatPass());
...
std::string HeartsState::InformationStateString(Player player) const {
  ...
  if (passed_cards_[0].size() > 0) absl::StrAppend(&rv, FormatPass(player));
  if (num_cards_played_ > 0) absl::StrAppend(&rv, FormatPlay(), FormatPoints());
  return rv;
}
...
std::vector<Action> HeartsState::PlayLegalActions() const {
  ...
  if (num_cards_played_ % kNumPlayers == 0) {
    ...
    // been broken.
    if (can_lead_hearts_instead_of_qs_ &&
