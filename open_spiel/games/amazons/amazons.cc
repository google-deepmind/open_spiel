...
const GameType kGameType{/*short_name=*/"amazons",
                         /*long_name=*/"Amazons",
...
bool AmazonsState::HasAnyMoveFromCell(Action cell) const {
  ...
  for (int d = 0; d < 8; ++d) {
    const int r = row + kDirRow[d];
    const int c = col + kDirCol[d];
...
AmazonsState::AmazonsState(std::shared_ptr<const Game> game, int board_size)
  ...
  switch (board_size_) {
    ...
    case 6:
      ...
      break;
    default:
      SpielFatalError(absl::StrCat("Unsupported board_size: ", board_size_));
  }
}
