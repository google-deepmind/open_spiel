...
template <uint32_t kBoardSize>
/*static*/ Optional<ChessBoard<kBoardSize>>
...
template <uint32_t kBoardSize>
Optional<Move> ChessBoard<kBoardSize>::ParseMove(
...
template <uint32_t kBoardSize>
Optional<Move> ChessBoard<kBoardSize>::ParseSANMove(
    const std::string &move_str) const {
  std::string move = move_str;

  if (move.empty()) return kNullopt;
  ...
  // disambiguation.
  Optional<int8_t> source_file, source_rank;
  if (!move.empty()) {
  ...
  std::cerr << "expected exactly one matching move, got " << candidates.size()
            << std::endl;
  return Optional<Move>();
}

template <uint32_t kBoardSize>
Optional<Move> ChessBoard<kBoardSize>::ParseLANMove(
    const std::string &move) const {
  SPIEL_CHECK_FALSE(move.empty());
