...
#include "open_spiel/games/nine_mens_morris.h"
...
constexpr std::array<std::array<int, 2>, kNumPoints> kPointStrCoords = {{
  {0, 0}, {0, 7}, {0, 14}, {2, 2}, {2, 7}, {2, 12},
  {4, 4}, {4, 7}, {4, 10}, {6, 0}, {6, 2}, {6, 4},
  {6, 10}, {6, 12}, {6, 14}, {8, 4}, {8, 7}, {8, 10},
  {10, 2}, {10, 7}, {10, 12}, {12, 0}, {12, 7}, {12, 14}
}};
...
NineMensMorrisState::NineMensMorrisState(std::shared_ptr<const Game> game) : State(game) {
...
bool NineMensMorrisState::IsTerminal() const {
  return num_turns_ >= kMaxNumTurns ||
         num_men_[0] <= 2 || num_men_[1] <= 2 ||
...
void NineMensMorrisState::ObservationTensor(Player player,
  ...
  TensorView<3> view(values,
      {kCellStates + 2, kObservationSize, kObservationSize}, true);
