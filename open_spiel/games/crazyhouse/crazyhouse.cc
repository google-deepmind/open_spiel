...
ActionsAndProbs CrazyhouseState::ChanceOutcomes() const {
...
std::unique_ptr<State> CrazyhouseState::Clone() const {
  return std::unique_ptr<State>(new CrazyhouseState(*this));
}
...
bool CrazyhouseState::IsRepetitionDraw() const {
  const auto entry = repetitions_.find(Board().HashValue());
  SPIEL_CHECK_FALSE(entry == repetitions_.end());
  return entry->second >= kNumRepetitionsToDraw;
}

int CrazyhouseState::NumRepetitions(const CrazyhouseState& state) const {
...
CrazyhouseState::ExtractFenAndMaybeMoves() const {
  ...
  CrazyhouseBoard board = down_cast<const CrazyhouseState&>(*state).Board();
  for (const Move& move : moves_history_) {
...
std::string CrazyhouseState::StartFEN() const {
...
int CrazyhouseGame::MaxChanceOutcomes() const {
  if (IsChess960()) {
    return 960;
  } else {
    return 0;
