...
bool IsLongDiagonal(const crazyhouse::Square &from_sq, const crazyhouse::Square &to_sq,
                    int board_size) {
  if (from_sq == to_sq) {
    return false;
  }
...
void CrazyhouseBoard::BreachingMoveToCaptureMove(Move* move) const {
...
bool CrazyhouseBoard::HasSufficientMaterial() const {
...
void CrazyhouseBoard::ApplyMove(const Move &move) {
  ...
  if (move == kPassMove) {
    ...
    SetEpSquare(crazyhouse::kInvalidSquare);
    return;
  }
...
std::string CrazyhouseBoard::DebugString(bool shredder_fen) const {
  std::string s;
...
std::string CrazyhouseBoard::ToUnicodeString() const {
  std::string out = "\n";
...
std::string CrazyhouseBoard::ToDarkFEN(const ObservationTable &observability_table,
                                  Color color) const {
  std::string fen;
  ...
  for (int8_t rank = board_size_ - 1; rank >= 0; --rank) {
    int num_empty = 0;
    for (int8_t file = 0; file < board_size_; ++file) {
      size_t index = SquareToIndex_(crazyhouse::Square{file, rank});
  ...
  // 2. color to play.
  absl::StrAppend(&fen, " ", ToPlay() == crazyhouse::Color::kWhite ? "w" : "b");

  // 3. by castling rights.
  absl::StrAppend(&fen, " ");
  std::string castling_rights;
  if (color == crazyhouse::Color::kWhite) {
    if (CastlingRight(crazyhouse::Color::kWhite, crazyhouse::CastlingDirection::kRight)) {
      castling_rights.push_back('K');
    }
    if (CastlingRight(crazyhouse::Color::kWhite, crazyhouse::CastlingDirection::kLeft)) {
      castling_rights.push_back('Q');
    }
  } else {
    if (CastlingRight(crazyhouse::Color::kBlack, crazyhouse::CastlingDirection::kRight)) {
      castling_rights.push_back('k');
    }
    if (CastlingRight(crazyhouse::Color::kBlack, crazyhouse::CastlingDirection::kLeft)) {
      castling_rights.push_back('q');
    }
  }
  absl::StrAppend(&fen, castling_rights.empty() ? "-" : castling_rights);
...
void CrazyhouseBoard::SetToPlay(Color c) {
...
void CrazyhouseBoard::SetIrreversibleMoveCounter(int c) {
  irreversible_move_counter_ = c;
}

void CrazyhouseBoard::SetMovenumber(int move_number) { move_number_ = move_number; }

bool CrazyhouseBoard::EpSquareThreatened(Square ep_square) const {
...
void CrazyhouseBoard::SetEpSquare(Square sq) {
...
std::string DefaultFen(int board_size) {
  if (board_size == 8)
    ...
    return crazyhouse::kDefaultSmallFEN;
  else
    SpielFatalError(
