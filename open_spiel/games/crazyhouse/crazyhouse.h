...
class CrazyhouseState : public State {
 public:
  ...
  // Starting board.
  CrazyhouseBoard& StartBoard() { return start_board_; }
  const CrazyhouseBoard& StartBoard() const { return start_board_; }
  ...
  // algebraic notation (using CrazyhouseBoard::ParseMove), or kInvalidAction if
  // the parsing fails.
  ...
  // history.
  int NumRepetitions(const CrazyhouseState& state) const;
  ...
  const CrazyhouseGame* ParentGame() const {
    return down_cast<const CrazyhouseGame*>(GetGame().get());
  }
...
class CrazyhouseGame : public Game {
 public:
  ...
  std::unique_ptr<State> NewInitialState(
    ...
    return absl::make_unique<CrazyhouseState>(shared_from_this(), fen);
  }
  std::unique_ptr<State> NewInitialState() const override {
    return absl::make_unique<CrazyhouseState>(shared_from_this());
  }
  int NumPlayers() const override { return crazyhouse::NumPlayers(); }
  ...
  std::vector<int> ObservationTensorShape() const override {
    return crazyhouse::ObservationTensorShape();
  }
  int MaxGameLength() const override { return crazyhouse::MaxGameLength(); }
