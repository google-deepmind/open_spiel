...
inline constexpr const int kStateEncodingSize = kNumPlayers * kNumPlayerCubes *
                                                kDefaultRows * kDefaultColumns;
...
struct TurnHistoryInfo {
  ...
  Action action;
  Cube captured_cube;
  TurnHistoryInfo(int _player, int _prev_player, int _die_roll,
                  int _action, Cube _captured_cube)
      : player(_player),
...
class EinsteinWurfeltNichtState : public State {
 public:
  ...
  void SetState(int cur_player, int die_roll,
              const std::array<Cube, k2dMaxBoardSize> board,
