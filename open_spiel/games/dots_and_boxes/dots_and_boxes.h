...
class Move {
  ...
  void SetRowsCols(int rows, int cols) {num_rows_ = rows; num_cols_ = cols;}
...
class DotsAndBoxesState : public State {
 public:
  DotsAndBoxesState(std::shared_ptr<const Game> game,
                    int num_rows, int num_cols, bool utility_margin);
  DotsAndBoxesState(std::shared_ptr<const Game> game,
                    int num_rows, int num_cols, bool utility_margin, const std::string& dbn);
...
class DotsAndBoxesGame : public Game {
 public:
  ...
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new DotsAndBoxesState(shared_from_this(),
                                                        num_rows_, num_cols_,
                                                        utility_margin_));
  }
  std::unique_ptr<State> NewInitialState(const std::string& str) const override {
    return absl::make_unique<DotsAndBoxesState>(shared_from_this(),
                                                num_rows_, num_cols_,
                                                utility_margin_, str);
  }
