...
const GameType kGameType{
    ...
    GameType::Utility::kZeroSum, //kGeneralSum,
...
Move::Move(Action action, int rows, int cols) {
  ...
  if (action < maxh) {
    ...
    //std::cout << "Action3[h," << row_ << "," << col_ << "] = " << action << std::endl;
  } else {
    // Vertical
    action -= maxh;
    orientation_ = CellOrientation::kVertical;
    ...
    //std::cout << "Action3[v," << row_ << "," << col_ << "] = " << action << std::endl;
  }
...
CellOrientation Move::GetOrientation() const {
  return orientation_;
}
...
Action Move::ActionId() {
  ...
  if (orientation_ == CellOrientation::kHorizontal) {
    ...
    std::cout << "Action2[h," << row_ << "," << col_ << "] = " << action << std::endl;
  } else {
    action = maxh + row_ * (num_cols_ + 1) + col_;
    std::cout << "Action2[v," << row_ << "," << col_ << "] = " << action << std::endl;
  }
  return action;
}
...
bool DotsAndBoxesState::IsFull() const {
    return num_moves_ == (num_rows_ + 1) * num_cols_ + num_rows_ * (num_cols_ + 1);
}

DotsAndBoxesState::DotsAndBoxesState(std::shared_ptr<const Game> game,
                                     int num_rows, int num_cols, bool utility_margin) :
  State(game),
  num_rows_(num_rows),
  num_cols_(num_cols),
...
std::string DotsAndBoxesState::StateToStringH(CellState state, int row, int col) const {
  if (row == 0 && col == 0) {
...
std::string DotsAndBoxesState::StateToStringV(CellState state, int row, int col) const {
  if (state == CellState::kEmpty) {
    return " ";//"┊";
  } else {
    return "│";
...
std::string DotsAndBoxesState::StateToStringP(CellState state, int row, int col) const {
  if (state == CellState::kEmpty) {
    return "   ";
  }
...
DotsAndBoxesGame::DotsAndBoxesGame(const GameParameters& params)
    : Game(kGameType, params),
      ...
      num_cells_((1 + ParameterValue<int>("num_rows", kDefaultNumRows)) * (1 + ParameterValue<int>("num_cols", kDefaultNumCols))),
      utility_margin_(ParameterValue<bool>("utility_margin", kDefaultUtilityMargin))
{
  //std::cout << "Init dots and boxes game\n";
  //if (utility_margin_) {
...
absl::optional<double> DotsAndBoxesGame::UtilitySum() const {
    return 0;
    //SpielFatalError(
    //    "Called `UtilitySum()` on a general Dots-and-Boxes game: "
    //    "set `utility_margin` to true for a zero-sum game.");
}
