...
GoColor OppColor(GoColor c) {
    switch (c) {
        case GoColor::kBlack:return GoColor::kWhite;
        case GoColor::kWhite:return GoColor::kBlack;
        case GoColor::kEmpty:
        case GoColor::kGuard:return c;
        default:SpielFatalError(absl::StrCat("Unknown color ", c, " in OppColor."));
            return c;
    }
}
...
std::string VirtualPointToString(VirtualPoint p) {
    switch (p) {
        case kInvalidPoint:return "INVALID_POINT";
        case kVirtualPass:return "PASS";
        default: {
...
int NumSurroundedPoints(const PhantomGoBoard &board, const VirtualPoint p,
    ...
    Neighbours(p, [&board, &num_points, marked, reached_black,
      ...
      switch (board.PointColor(n)) {
          case GoColor::kBlack:*reached_black = true;
              break;
          case GoColor::kWhite:*reached_white = true;
              break;
          case GoColor::kEmpty:
              num_points +=
...
float TrompTaylorScore(const PhantomGoBoard &board, float komi, int handicap) {
    ...
    for (VirtualPoint p: BoardPoints(board.board_size())) {
        switch (board.PointColor(p)) {
            case GoColor::kBlack:++occupied_delta;
                break;
            case GoColor::kWhite:--occupied_delta;
                break;
            case GoColor::kEmpty: {
            ...
            case GoColor::kGuard:SpielFatalError("unexpected color");
        }
    }
