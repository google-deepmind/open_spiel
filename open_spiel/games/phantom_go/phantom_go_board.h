...
//
// Normal go is standardly played on board of size 19, for Phantom Go, standard
// is size 9
...
class PhantomGoBoard {
 public:
  ...
  std::array<GoColor, kMaxBoardSize * kMaxBoardSize> GetObservationByID(
      int player_id) const;
 ...
 private:
  ...
  // In this context, GoColor::kEmpty suggests, that a player does not know,
  // what piece is on that exact spot
  std::array<std::array<GoColor, kMaxBoardSize * kMaxBoardSize>, 2>
      observations_;

  // On index 0 is stored count of black stones, on index 1 is stored count of
  // white stones so it equals the enum of GoColor, where kBlack is 0
...
float TrompTaylorScore(const PhantomGoBoard &board, float komi,
                       int handicap = 0);
