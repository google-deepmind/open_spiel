...
extern "C" int ROSHAMBO_BOT_my_history[kNumThrows + 1];
extern "C" int ROSHAMBO_BOT_opp_history[kNumThrows + 1];
...
RoshamboBot::RoshamboBot(Player player_id, std::string bot_name)
    : player_id_(player_id),
      ...
      my_history_{0},
      opp_history_{0} {
  if (::roshambo_tournament::bot_map.find(bot_name) ==
      ::roshambo_tournament::bot_map.end())
...
Action RoshamboBot::Step(const State& /*state*/) {
  ...
  Action action = ::roshambo_tournament::bot_map[bot_name_]();
