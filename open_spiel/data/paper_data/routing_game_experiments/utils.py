...
def create_games(origin,
  ...
  game = dynamic_routing.DynamicRoutingGame(
      {'max_num_time_step': max_time_step, 'time_step_length': time_step_length},
      network=graph,
  ...
  while not state.is_terminal():
    ...
    if state.is_chance_node():
      ...
      if policy is not None:
        ...
        if debug:
          print(
              (f"Policy for player {state.current_player()} at location "
               f"{vehicle_location[state.current_player()]}: ") +
              str([(str(graph.get_road_section_from_action_id(k)) +
                f'with probability {v}') for k, v in state_policy.items()])
          )
