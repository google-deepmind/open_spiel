...
std::shared_ptr<const GameWithPublicStates> Factory(
  ...
  return std::make_shared<KuhnGameWithPublicStates>(
      down_cast<const KuhnGame>(game));
}
...
KuhnGameWithPublicStates::KuhnGameWithPublicStates(
    std::shared_ptr<const KuhnGame> base_game)
    : GameWithPublicStates(base_game), kuhn_game_(base_game) {}
...
std::vector<ReachProbs> KuhnGameWithPublicStates::NewInitialReachProbs()
const {
  auto probs = std::vector<ReachProbs>();
  probs.reserve(kuhn_game_->NumPlayers());

  for (int i = 0; i < kuhn_game_->NumPlayers(); ++i) {
...
int KuhnGameWithPublicStates::NumPublicFeatures() const {
  ...
  return kuhn_game_->NumPlayers() +  // First round.
      kuhn_game_->NumPlayers() - 1;  // Second round.
}

std::vector<int> KuhnGameWithPublicStates::MaxDistinctPrivateInformationsCount()
const {
  int max_cards = kuhn_game_->NumPlayers() + 1;
  return std::vector<int>(kuhn_game_->NumPlayers(), max_cards);
}

// KuhnPrivateInformation ------------------------------------------------------

inline constexpr const int kNoCardDealt = -1;
...
KuhnPrivateInformation::KuhnPrivateInformation(
    ...
    : PrivateInformation(base_game),
      kuhn_game_(down_cast<const KuhnGame>(base_game)),
      player_(player), player_card_(player_card) {
  SPIEL_CHECK_GT(player_, 0);
  SPIEL_CHECK_LT(player_, kuhn_game_->NumPlayers());
  SPIEL_CHECK_GT(player_card_, 0);
  SPIEL_CHECK_LT(player_card_, kuhn_game_->NumPlayers() + 1);
}
Player KuhnPrivateInformation::GetPlayer() const {
  return player_;
}
int KuhnPrivateInformation::GetPlayerCard() const {
  return player_card_;
}
unsigned int KuhnPrivateInformation::ReachProbsIndex() const {
  return player_card_ == kNoCardDealt ? 0 : player_card_;
}
unsigned int KuhnPrivateInformation::NetworkIndex() const {
  return player_card_ == kNoCardDealt ? 0 : player_card_;
}
bool KuhnPrivateInformation::IsStateCompatible(const State& state) const {
  const auto kuhn_state = down_cast<const KuhnState&>(state);
  if (kuhn_state.History().size() < player_ && player_card_ == kNoCardDealt)
    return true;
  if (kuhn_state.CardDealt().at(player_) == player_card_)
    return true;
  return false;
}
std::string KuhnPrivateInformation::ToString() const {
  if (player_card_ == kNoCardDealt) {
    return absl::StrCat("Player ", std::to_string(player_), " has no Card.");
  }

  return absl::StrCat(
      "Player ", std::to_string(player_),
      " has Card ", std::to_string(player_card_));
}
std::unique_ptr<PrivateInformation> KuhnPrivateInformation::Clone() const {
  return std::make_unique<KuhnPrivateInformation>(
      kuhn_game_, player_, player_card_);
}
std::string KuhnPrivateInformation::Serialize() const {
  return absl::StrCat(std::to_string(player_), "-",
                      std::to_string(player_card_));
}
bool KuhnPrivateInformation::operator==(const PrivateInformation& other) const {
  const auto& other_kuhn = down_cast<const KuhnPrivateInformation&>(other);
...
int KuhnPublicState::NumPlayers() const {
  return base_game_->NumPlayers();
}
int KuhnPublicState::NumCards() const {
  return base_game_->NumPlayers() + 1;
}

KuhnPublicState::KuhnPublicState(
    std::shared_ptr<const GameWithPublicStates> public_game)
    : PublicState(public_game),
      kuhn_game_(down_cast<const KuhnGame>(public_game->GetBaseGame())) {}
...
std::vector<PrivateInformation> KuhnPublicState::GetPrivateInformations(
  ...
  if (!PlayerHasSeenHisCard(player)) {
    return {
        KuhnPrivateInformation(base_game_, player, kNoCardDealt)
    };
  }
  ...
  for (int i = 0; i < NumCards(); ++i) {
    private_infos.push_back(
        KuhnPrivateInformation(base_game_, player, i));
  }
...
std::vector<std::unique_ptr<State>> KuhnPublicState::GetPublicSet() const {
  ...
  std::vector<std::vector<int>> trajectories = VariationsWithoutRepetition(
      cards, std::fmin(GetDepth(), NumPlayers()));

  std::vector<std::unique_ptr<State>> states;
  states.reserve(trajectories.size());

  for (auto& trajectory : trajectories) {
    auto s = std::make_unique<KuhnState>(base_game_);
...
std::string KuhnPublicState::GetInformationState(
  ...
  const auto& kuhn_information =
      down_cast<const KuhnPrivateInformation&>(information);
  std::string info_state = std::to_string(kuhn_information.GetPlayerCard());
  for (int i = 0; i < public_actions_.size(); ++i) {
    absl::StrAppend(&info_state, public_actions_[i] == kBet ? "b" : "p");
  }
  return info_state;
}
std::vector<State> KuhnPublicState::GetInformationSet(
    const PrivateInformation& information) const {
  const auto& kuhn_information =
      down_cast<const KuhnPrivateInformation&>(information);
...
std::unique_ptr<State> KuhnPublicState::GetWorldState(
  ...
  for (int i = 0; i < informations.size(); ++i) {
    auto kuhn_information =
        down_cast<KuhnPrivateInformation*>(informations[i]);
...
std::unique_ptr<State> KuhnPublicState::ResampleFromPublicSet(
  ...
  for (int i = 0; i < std::fmin(NumPlayers(), GetDepth()); ++i) {
    ...
    const auto&[action, prob] = SampleAction(
        state->ChanceOutcomes(), random->RandomUniform());
...
std::vector<PublicTransition> KuhnPublicState::GetPublicTransitions() const {
  ...
  if (GetDepth() < 2 * NumPlayers()) {
    return {
        std::to_string(ActionType::kPass),
        std::to_string(ActionType::kBet)
    };
  }
  // Second round.
  int num_passes_without_bet = 0;
  for (int i = 0; i < GetDepth() - NumPlayers(); ++i) {
    if (public_actions_[i] == ActionType::kBet) break;
    SPIEL_CHECK_EQ(public_actions_[i], ActionType::kPass);
...
std::vector<std::vector<Action>> KuhnPublicState::GetPrivateActions(
  ...
  return std::vector<std::vector<Action>>{
      NumCards(), {ActionType::kPass, ActionType::kBet}
  };
}
...
std::vector<double> KuhnPublicState::PublicFeaturesTensor() const {
  auto tensor = std::vector<double>(
      public_game_->NumPublicFeatures(), kTensorUnusedSlotValue);
  SPIEL_CHECK_LE(public_actions_.size(), tensor.size());
  std::copy(public_actions_.begin(), public_actions_.end(),
            tensor.begin());
  return tensor;
}
ReachProbs KuhnPublicState::ComputeReachProbs(
  ...
  ActionType type = static_cast<ActionType>(std::stoi(transition));
...
std::vector<CfPrivValues> KuhnPublicState::TerminalCfValues(
  ...
  bool both_players_have_bet = public_actions_.back() == kBet;
...
void KuhnPublicState::DoApplyPublicTransition(
  ...
  public_actions_.push_back(static_cast<ActionType>(std::stoi(transition)));
}
