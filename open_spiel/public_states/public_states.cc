...
PublicInformation::PublicInformation(std::shared_ptr<const Game> game)
    : game_(std::move(game)) {
//  SPIEL_CHECK_TRUE(game_->GetType().provides_factored_observation_string);
}
...
GameWithPublicStatesRegisterer::CreateByGame(
  ...
  if (iter == factories().end()) {
    SpielFatalError(absl::StrCat(
        "The game '", short_name, "' does not have a public state API. "
...
std::shared_ptr<const GameWithPublicStates> LoadGameWithPublicStates(
  ...
  if (result == nullptr) {
    SpielFatalError(absl::StrCat(
        "Unable to create game with public state API: ", name));
  }
  return result;
}
...
std::string SerializeGameWithPublicState(
  ...
  // TODO(sustr): implement
  SpielFatalError(
      "SerializeGameWithPublicState() is not implemented yet.");
}
...
DeserializeGameWithPublicState(const std::string &serialized_state) {
  // TODO(sustr): implement
  SpielFatalError(
      "DeserializeGameWithPublicState() is not implemented yet.");
}
