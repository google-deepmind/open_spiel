...
bool CreateGraphDef(const Game& game, double learning_rate,
  ...
  return RunPython("open_spiel.algorithms.alpha_zero.cc.export_vpnet",
                   {
...
VPNetModel::VPNetModel(const Game& game, const std::string& path,
  ...
  TF_CHECK_OK(
      ReadBinaryProto(tf::Env::Default(), absl::StrCat(path, "/", file_name),
                      &meta_graph_def_));
...
VPNetModel::LossInfo VPNetModel::Learn(const std::vector<TrainInputs>& inputs) {
  ...
  for (int b = 0; b < training_batch_size; ++b) {
    const std::vector<int>& mask = inputs[b].legal_action_masks;
    ...
    for (int a = 0; a < inputs[b].policy_targets.size(); ++a) {
      policy_targets_matrix(b, a) = inputs[b].policy_targets[a];
    }

    // Value targets. Assume value net always in view of player 0.
    value_targets_matrix(b, 0) = inputs[b].returns[0];
  }
