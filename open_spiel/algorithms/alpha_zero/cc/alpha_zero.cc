...
#include "open_spiel/algorithms/alpha_zero/cc/alpha_zero.h"
...
#include "open_spiel/algorithms/alpha_zero/cc/device_manager.h"
#include "open_spiel/algorithms/alpha_zero/cc/vpevaluator.h"
...
Trajectory PlayGame(
  ...
  while (true) {
    ...
    trajectory.states.push_back(ReplayNode{
        ...
        state->LegalActions(), state->LegalActionsMask(), action,
...
void learner(const open_spiel::Game& game,
  ...
  for (int step = 1; !stop->StopRequested() &&
    ...
    {  // Extra scope to return the device for use for inference asap.
      ...
      for (int i = 0; i < replay_buffer.Size() / config.train_batch_size; i++) {
        ...
        for (const ReplayNode& node : data) {
          std::vector<double> state_policy_targets(
              node.legal_actions_mask.size(), 0);
          for (const auto& [action, prob] : node.policy)
            state_policy_targets[action] = prob;
          train_inputs.emplace_back(
              VPNetModel::TrainInputs{
                  node.legal_actions_mask,
                  node.observation,
                  state_policy_targets,
                  node.returns});
        }
    ...
    if (step % config.checkpoint_freq == 0 || device_manager->Count() > 0) {
      ...
      } else if (!file::Remove(checkpoint_path + ".index") ||
                 !file::Remove(checkpoint_path + ".data-00000-of-00001")) {
        logger.Print("Failed to delete checkpoint: %s", checkpoint_path);
      }
    }
...
uint32_t RoundDownToPower2(uint32_t x) {
    x = x | (x >> 1);
    x = x | (x >> 2);
    x = x | (x >> 4);
    x = x | (x >> 8);
...
bool AlphaZero(AlphaZeroConfig config, StopToken* stop) {
  if (config.inference_batch_size == 0) {
    config.inference_batch_size =
        std::max(1, static_cast<int>(RoundDownToPower2(
            config.actors + config.evaluators)) / 4);
    if (config.inference_threads == 0) {
      config.inference_threads = 4;
    }
  }
