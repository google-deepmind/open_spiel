...
bool SaveModelConfig(const std::string& path,
...
bool CreateGraphDef(const Game& game,
...
VPNetModel::VPNetModel(const Game& game,
                       const std::string& path,
...
void VPNetModel::LoadCheckpoint(const std::string& path) {
  ...
  torch::load(
      model_optimizer_, absl::StrCat(path, "-optimizer.pt"), torch_device_);
}
...
std::vector<VPNetModel::InferenceOutputs> VPNetModel::Inference(
  ...
  torch::Tensor torch_inf_inputs = torch::empty(
      {inference_batch_size, flat_input_size_}, torch_device_);
  ...
  // Run the inference.
  model_->eval();
  std::vector<torch::Tensor> torch_outputs = model_(
    torch_inf_inputs, torch_inf_legal_mask);
...
VPNetModel::LossInfo VPNetModel::Learn(const std::vector<TrainInputs>& inputs) {
  ...
  torch::Tensor torch_train_inputs = torch::empty(
      {training_batch_size, flat_input_size_}, torch_device_);
  torch::Tensor torch_train_legal_mask = torch::full(
      {training_batch_size, num_actions_}, false,
      torch::TensorOptions().dtype(torch::kByte).device(torch_device_));
  torch::Tensor torch_policy_targets = torch::zeros(
      {training_batch_size, num_actions_}, torch_device_);
  torch::Tensor torch_value_targets = torch::empty(
      {training_batch_size, 1}, torch_device_);
  ...
  std::vector<torch::Tensor> torch_outputs = model_->losses(
      torch_train_inputs, torch_train_legal_mask,
      torch_policy_targets, torch_value_targets);

  torch::Tensor total_loss = torch_outputs[0] +
                             torch_outputs[1] +
                             torch_outputs[2];
