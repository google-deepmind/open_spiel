...
ResTorsoBlockImpl::ResTorsoBlockImpl(
    const ResTorsoBlockConfig& config, int layer)
    : conv1_(
          torch::nn::Conv2dOptions(
              /*input_channels=*/config.input_channels,
              /*output_channels=*/config.filters,
  ...
  register_module(
    "res_" + std::to_string(layer) + "_batch_norm_1", batch_norm1_);
  register_module(
    "res_" + std::to_string(layer) + "_batch_norm_2", batch_norm2_);
}
...
ResModelImpl::ResModelImpl(const ModelConfig& config, const std::string& device)
  ...
  ResInputBlockConfig input_config = {
  ...
  ResTorsoBlockConfig residual_config = {
...
std::vector<torch::Tensor> ResModelImpl::losses(
  ...
  torch::Tensor value_loss = mse_loss(
  ...
  for (auto& named_parameter : this->named_parameters()) {
    ...
    if (parameter_name.find("bias") != std::string::npos) { continue; }

    // Copy TensorFlow's l2_loss function.
    // https://www.tensorflow.org/api_docs/python/tf/nn/l2_loss      
    l2_regularization_loss +=
        weight_decay_ * torch::sum(
            torch::square(named_parameter.value())) / 2;
  }
