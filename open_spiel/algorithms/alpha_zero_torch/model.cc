...
MLPOutputBlockImpl::MLPOutputBlockImpl(const int nn_width, const int policy_linear_out_features)
        : value_linear1_(torch::nn::LinearOptions(
          ...
          policy_linear1_(torch::nn::LinearOptions(
                  ...
                  /*output_channels=*/nn_width).bias(true)),
...
ModelImpl::ModelImpl(const ModelConfig &config, const std::string &device)
    ...
    if (config.nn_model == "resnet") {
        ...
        layers_->push_back(MLPOutputBlock(config.nn_width,config.number_of_actions));
