...
void TestModelCreation() {
  ...
  ModelConfig net_config = {/*observation_tensor_shape=*/game->ObservationTensorShape(),
...
void TestModelInference() {
  ...
  const int rows = 8;
  const int columns = 8;
  std::string game_string = absl::StrCat("clobber(rows=",
                                         std::to_string(rows),
                                         ",columns=",
  ...
  torch::Tensor mask = torch::full(
    {1, game->NumDistinctActions()}, false,
  ...
  std::cout << "Input:\n"
            << observation_tensor.view({channels, rows, columns})
            << std::endl;
  std::cout << "Mask:\n" << mask << std::endl;
