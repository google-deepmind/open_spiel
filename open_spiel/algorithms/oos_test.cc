...
// TODO(sustr): merge with functional.h
...
void EpsExploreSamplingPolicyTest() {
  ...
  auto chn_3cards_dist = ZipActionsProbs(
      card_to_player0->LegalActions(), {1 / 3., 1 / 3., 1 / 3.});
  auto chn_2cards_dist = ZipActionsProbs(
      card_to_player1->LegalActions(), {1 / 2., 1 / 2.});
  auto player0_dist = ZipActionsProbs(
      player0_plays->LegalActions(), {1 / 2., 1 / 2.});
  auto player1_dist = ZipActionsProbs(
      player1_plays->LegalActions(), {1 / 2., 1 / 2.});

  std::vector<double> current_policy = {0.2, 0.8};
  auto expected_mix = ZipActionsProbs(
      player0_plays->LegalActions(), {
...
void UnbiasedIterationsConverge(
    std::shared_ptr<const Game> game, int iterations, double expl_bound) {
...
void BiasedIterationsConverge(
  ...
  for (std::unique_ptr<State>& state : states) {
    ...
    for (int player = 0; player < game->NumPlayers(); player++) {
      ...
      ActionObservationHistory aoh(player, *state);
    ...
    // Public-Observation history targeting:
    {
      ...
      PublicObservationHistory poh(*state);
...
void UnbiasedIterationsConvergeInGames() {
  UnbiasedIterationsConverge(LoadGame("coordinated_mp"), 100000, 0.01);
  UnbiasedIterationsConverge(LoadGame("kuhn_poker"), 100000, 0.01);
}

void BiasedIterationsConvergeInGames() {
  BiasedIterationsConverge(LoadGame("coordinated_mp"), 10000, 0.05);
  BiasedIterationsConverge(LoadGame("kuhn_poker"), 10000, 0.05);
}
