...
bool IsValidProbDistribution(const ActionsAndProbs& probs) {
  ...
  for (int i = 0; i < probs.size(); ++i) {
    if (probs[i].second < 0) return false;
    if (probs[i].second > 1) return false;
    sum_probs += probs[i].second;
  }
...
void
TargetedPolicy::UpdateTarget(const PublicObservationHistory* public_state) {
  NoTargeting();  // Reset.
...
ActionsAndProbs TargetedPolicy::GetStatePolicy(const State& h) const {
  ...
  ActionsAndProbs unbiased_policy;
  if (h.IsChanceNode()) {
    unbiased_policy = h.ChanceOutcomes();
  } else if (h.IsPlayerNode()) {
    unbiased_policy = CFRCurrentPolicy::GetStatePolicy(h);
  } else {
    SpielFatalError("Could not get policy for this state.");
    return;  // This will never return.;
  }
  ...
  ActionsAndProbs biased_policy = unbiased_policy;
  for (auto&[action, prob] : biased_policy) {
  ...
  if (biased_sum > 0) {
    for (auto&[_, prob] : biased_policy) prob /= biased_sum;
    policy = &biased_policy;
  } else {
    policy = &unbiased_policy;
  }

  // Mix in exploration.
  const double unif = 1. / policy->size();
  for (auto&[_, prob] : *policy) {
    prob = bias_exploration_ * unif + (1 - bias_exploration_) * prob;
  }
  return *policy;
}

bool TargetedPolicy::IsAllowedAction(
    const State& h, const Action& action) const {
  if (targeting_ == kDoNotUseTargeting)
    return true;

  const std::unique_ptr<State> ha = h.Child(action);

  if (targeting_ == Targeting::kInfoStateTargeting) {
    SPIEL_CHECK_NE(target_info_state_, kNoActionObsTargetSpecified);
    return target_info_state_->IsExtensionOf(
        target_info_state_->GetPlayer(), *ha);
  }
...
bool TargetedPolicy::IsTargetHit(const State& h) {
  ...
  SPIEL_CHECK_TRUE(targeting_ != kPublicStateTargeting
                       || target_public_state_ != kNoPublicObsTargetSpecified);
  const bool hit_info_state = targeting_ == kInfoStateTargeting
...
void OnlineStats::Print(std::ostream& out) {
  out << "Root visits:          " << root_visits << "\n";
  out << "State visits:         " << state_visits << "\n";
  out << "Terminal visits:      " << terminal_visits << "\n";
  out << "Rollouts (terminals): " << rollouts << "\n";
  out << "Target visits:        " << target_visits << "\n";
  out << "Target biased visits: " << target_biased_visits << "\n";
  out << "Biased iterations:    " << biased_iterations << std::endl;
  CheckConsistency();
}

void OnlineStats::CheckConsistency() {
  SPIEL_DCHECK_EQ(root_visits, terminal_visits + rollouts);
  SPIEL_DCHECK_LE(root_visits, state_visits);
  SPIEL_DCHECK_LE(target_biased_visits, target_visits);
}
...
double OOSAlgorithm::Iteration(State* h, double rm_h_pl, double rm_h_opp,
  ...
  if (h->IsChanceNode()) {
    return IterationChanceNode(
        h, rm_h_pl, rm_h_opp, bs_h_all, us_h_all, us_h_cn, exploringPl);
  }

  if (h->IsPlayerNode()) {
    return IterationPlayerNode(
        h, rm_h_pl, rm_h_opp, bs_h_all, us_h_all, us_h_cn, exploringPl);
  }

  SpielFatalError("Unrecognized state type.");
  return 0.;  // "This will never return.";
}

double OOSAlgorithm::IterationTerminalNode(
    State* h, double bs_h_all, double us_h_all, Player exploringPl) {
...
double
OOSAlgorithm::IterationChanceNode(State* h, double rm_h_pl, double rm_h_opp,
                                  double bs_h_all,
                                  double us_h_all, double us_h_cn,
                                  Player exploringPl) {
  SPIEL_DCHECK_TRUE(h->IsChanceNode());

  const TakeAction take = SelectAction(
      h, IsBiasingApplicable(bs_h_all));
  const double s_ha_all = Bias(take.bs, take.us);
  SPIEL_DCHECK_GT(s_ha_all, 0);

  h->ApplyAction(take.action);
  const double u_ha = Iteration(
      h, rm_h_pl, rm_h_opp,
      bs_h_all * take.bs,
      us_h_all * take.us,
      us_h_cn * take.us,
...
double
OOSAlgorithm::IterationPlayerNode(State* h, double rm_h_pl, double rm_h_opp,
                                  double bs_h_all,
                                  double us_h_all, double us_h_cn,
                                  Player exploringPl) {
  ...
  const PlayerNodeOutcome outcome =
      is_leaf_state ? IncrementallyBuildTree(h, info_state,
                                             s_h_all, exploringPl)
                    : SampleExistingTree(h, info_state, &it->second,
                                         rm_h_pl, rm_h_opp,
  ...
  if (exploring_move_in_node) {
    UpdateInfoStateCumulativeRegrets(
        &outcome.data, outcome.action, outcome.u_ha, outcome.u_h,
...
PlayerNodeOutcome
OOSAlgorithm::SampleExistingTree(State* h, const std::string& info_state,
                                 CFRInfoStateValues* values, double rm_h_pl,
  ...
  const unsigned int action_index = values->GetActionIndex(take.action);
  ...
  const double u_ha = Iteration(
      h,
      (exploring_move_in_node) ? rm_h_pl * rm_ha_both : rm_h_pl,
      (exploring_move_in_node) ? rm_h_opp : rm_h_opp * rm_ha_both,
      bs_h_all * take.bs, us_h_all * take.us, us_h_cn,
      exploringPl);

  double u_h = u_ha * rm_ha_both / s_ha_all;
  SPIEL_DCHECK_FALSE(std::isnan(rm_ha_both));
  SPIEL_DCHECK_FALSE(std::isnan(u_h));
  return PlayerNodeOutcome{
      take.action, rm_ha_both, u_h, u_ha / s_ha_all, *values};
}

PlayerNodeOutcome OOSAlgorithm::IncrementallyBuildTree(
    State* h, const std::string& info_state,
  ...
  const auto[it, state_inserted] = values_->emplace(
      info_state, CFRInfoStateValues(actions));
...
void OOSAlgorithm::UpdateInfoStateCumulativePolicy(
  ...
  for (unsigned int i = 0; i < values->cumulative_policy.size(); i++) {
...
void OOSAlgorithm::UpdateInfoStateCumulativeRegrets(
    CFRInfoStateValues* values, Action a,
    double u_ha, double u_h, double importance_sampling_ratio) {
  ...
  const unsigned int action_index = values->GetActionIndex(a);
