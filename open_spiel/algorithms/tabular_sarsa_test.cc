...
double
PlayCatch(absl::flat_hash_map<std::pair<std::string, Action>, double> q_values,
...
void TabularSarsaTest_Catch_Lambda00_Loss() {
  // Classic SARSA. No bootstraping (lambda=0.0)
  // Player loses after only 1 train iteration.
  std::shared_ptr<const Game> game = LoadGame("catch");
  open_spiel::algorithms::TabularSarsaSolver tabular_sarsa_solver(
      game, -1.0, 0.1, 0.01, 0.99, 0);
...
void TabularSarsaTest_Catch_Lambda00_Win() {
  // Classic SARSA. No bootstraping (lambda=0.0)
...
void TabularSarsaTest_Catch_Lambda01_Win() {
  // Player wins after 100 train iterations
  std::shared_ptr<const Game> game = LoadGame("catch");
  open_spiel::algorithms::TabularSarsaSolver tabular_sarsa_solver(
...
void TabularSarsaTest_Catch_Lambda01FasterThanLambda00() {
  // Eligibility traces (lambda > 0.0) always achieves victory with less
  // training steps w.r.t. SARSA(lambda=0.0)
  std::shared_ptr<const Game> game = LoadGame("catch");
  open_spiel::algorithms::TabularSarsaSolver tabular_sarsa_solve_lambda00(
      game, -1.0, 0.1, 0.01, 0.99, 0.0);
  open_spiel::algorithms::TabularSarsaSolver tabular_sarsa_solve_lambda01(
      game, -1.0, 0.1, 0.01, 0.99, 0.1);
  ...
  for (int seed = 0; seed < 100; seed++) {
    ...
    while (lambda_00_reward == -1.0) {
      tabular_sarsa_solve_lambda00.RunIteration();
      std::unique_ptr<State> state = game->NewInitialState();
      lambda_00_reward =
          PlayCatch(tabular_sarsa_solve_lambda00.GetQValueTable(), state, seed);
      lambda_00_train_iter++;
    }
    while (lambda_01_reward == -1.0) {
      tabular_sarsa_solve_lambda01.RunIteration();
      std::unique_ptr<State> state = game->NewInitialState();
      lambda_01_reward =
          PlayCatch(tabular_sarsa_solve_lambda01.GetQValueTable(), state, seed);
...
void TabularSarsaTest_TicTacToe_Lambda01_Win() {
  ...
  while (!state->IsTerminal()) {
    ...
    if (random_action == kInvalidAction)
      break;
...
void TabularSarsaTest_TicTacToe_Lambda01_Tie() {
  std::shared_ptr<const Game> game = open_spiel::LoadGame("tic_tac_toe");
  open_spiel::algorithms::TabularSarsaSolver tabular_sarsa_solver(
      game, -1.0, 0.1, 0.01, 0.99, 0.1);
...
int main(int argc, char **argv) {
  open_spiel::TabularSarsaTest_Catch_Lambda00_Loss();
  open_spiel::TabularSarsaTest_Catch_Lambda00_Win();
  open_spiel::TabularSarsaTest_Catch_Lambda01_Win();
  open_spiel::TabularSarsaTest_Catch_Lambda01FasterThanLambda00();
  open_spiel::TabularSarsaTest_TicTacToe_Lambda01_Win();
  open_spiel::TabularSarsaTest_TicTacToe_Lambda01_Tie();
}
