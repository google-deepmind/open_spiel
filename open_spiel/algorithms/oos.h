 ...
 * The player receives his current play position (infostate) and he has some
...
class ExplorativeSamplingPolicy: public CFRCurrentPolicy {
 public:
  ...
  ExplorativeSamplingPolicy(
    ...
    // We need the exploration to be positive
    // to guarantee all leaves are reachable.
    SPIEL_CHECK_GT(exploration_, 0);
    SPIEL_CHECK_PROB(exploration_);
  }
...
//
// The policy will give probability -0 to actions that do not lead to the target
// and a convex combination of bias_exploration between regret matching
// strategy and uniform strategy on actions to do lead to the target.
//
...
class TargetedPolicy: public CFRCurrentPolicy {
 protected:
  ...
  const ActionObservationHistory* target_info_state_ = kNoActionObsTargetSpecified;
  const PublicObservationHistory* target_public_state_ = kNoPublicObsTargetSpecified;
 ...
 public:
  TargetedPolicy(std::shared_ptr<const Game> game,
      ...
      : CFRCurrentPolicy(table, std::make_shared<UniformPolicy>()),
        game_(game), bias_exploration_(bias_exploration) {}
...
struct OnlineStats {
    unsigned int root_visits = 0;
    unsigned int state_visits = 0;
    unsigned int terminal_visits = 0;
    unsigned int rollouts = 0;
    unsigned int target_visits = 0;
    unsigned int target_biased_visits = 0;
    unsigned int biased_iterations = 0;
...
class OOSAlgorithm {
 public:
  ...
  std::unique_ptr<Policy> AveragePolicy() const {
    return std::unique_ptr<Policy>(
        new CFRAveragePolicy(*values_, default_policy_));
  }
  ...
  std::unique_ptr<Policy> CurrentPolicy() const {
    return std::unique_ptr<Policy>(
        new CFRCurrentPolicy(*values_, default_policy_));
  }

  // Maintain some stats for debugging purposes.
  // When needed, you can call Reset() to start counting from the start again.
  OnlineStats stats_;
 ...
 protected:
  ...
  double Iteration(State* h, double rm_h_pl, double rm_h_opp,
                   double bs_h_all, double us_h_all, double us_h_cn,
                   Player exploringPl);

  double IterationTerminalNode(
      State* h, double bs_h_all, double us_h_all, Player exploringPl);

  double IterationChanceNode(
      State* h, double rm_h_pl, double rm_h_opp,
      double bs_h_all,
      double us_h_all, double us_h_cn,
      Player exploringPl);

  double IterationPlayerNode(
      State* h, double rm_h_pl, double rm_h_opp,
      double bs_h_all,
      double us_h_all, double us_h_cn,
      Player exploringPl);
  ...
  PlayerNodeOutcome IncrementallyBuildTree(
      State* h, const std::string& info_state,
      double s_h_all, Player exploringPl);

  PlayerNodeOutcome SampleExistingTree(
