...
constexpr const char* kSerializeCFRSolverStateSectionHeader =
    "[CFRSolverState]";
...
CFRSolverBase::CFRSolverBase(const Game& game, bool alternating_updates,
                             bool linear_averaging, bool regret_matching_plus,
                             int iteration, CFRInfoStateValuesTable info_states)
    : game_(game),
      root_state_(game.NewInitialState()),
      root_reach_probs_(game_.NumPlayers() + 1, 1.0),
      regret_matching_plus_(regret_matching_plus),
...
std::string CFRSolverBase::Serialize(std::string delimiter) const {
  ...
  // Meta section
  absl::StrAppend(
      &str, "# Automatically generated by OpenSpiel "
  ...
  absl::StrAppend(&str, kSerializeCFRSolverStateSectionHeader, "\n");
  absl::StrAppend(&str, SerializeThisType(), "\n", iteration_, "\n");
  absl::StrAppend(&str,
      SerializeCFRInfoStateValuesTable(info_states_, delimiter));
...
CFRSolver DeserializeCFRSolver(const std::string& str, const Game& game,
  ...
  std::vector<absl::string_view> lines =
      DeserializeCFRSolverStateSection(str, game);
  SPIEL_CHECK_EQ(std::string(lines.front()), "CFRSolver");
  int iteration;
  absl::SimpleAtoi(lines[1], &iteration);
  return CFRSolver(game, iteration, DeserializeCFRInfoStateValuesTable(
...
CFRPlusSolver DeserializeCFRPlusSolver(const std::string& str, const Game& game,
  ...
  int iteration;
  absl::SimpleAtoi(lines[1], &iteration);
  return CFRPlusSolver(game, iteration, DeserializeCFRInfoStateValuesTable(
      absl::StrJoin(lines.begin() + 2, lines.end(), "\n"), delimiter));
}
...
std::vector<absl::string_view> DeserializeCFRSolverStateSection(
  ...
  std::array<std::string, 2> section_strings = {"", ""};
  ...
  for (i = 0; i < lines.size(); i++) {
    ...
    } else if (lines[i] == kSerializeCFRSolverStateSectionHeader) {
