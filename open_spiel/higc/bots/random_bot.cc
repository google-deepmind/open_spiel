...
void RandomBotMainLoop() {
  ...
  std::shared_ptr<Observer>
      public_observer = game->MakeObserver(kPublicObsType, {});
  std::shared_ptr<Observer>
      private_observer = game->MakeObserver(kPrivateObsType, {});
  ...
  while (true) {
    ...
    int score = 0;
    std::from_chars(message.data() + 11, message.data() + message.size(), score);
...
int main(int argc, char** argv) {
  open_spiel::higc::RandomBotMainLoop();
}
