...
void SimulateGame(std::mt19937 *rng,
  ...
  std::string state_prefix = fixed ? "Rnr state string of state in fixed part with underlying state:\n" :
  ...
  while (!normal_state->IsTerminal()) {
    SPIEL_CHECK_EQ(state_prefix + normal_state->ToString(), rnr_state->ToString());
    if (game.GetType().provides_information_state_string) {
      // Check the information states to each player are consistent.
      for (auto p = Player{0}; p < game.NumPlayers(); p++) {
        SPIEL_CHECK_EQ((p == fixed_player ? infostate_prefix : "") + normal_state->InformationStateString(p),
    ...
    if (normal_state->IsChanceNode()) {
      ...
      Action action =
          open_spiel::SampleAction(
              outcomes, std::uniform_real_distribution<double>(0.0, 1.0)(*rng)).first;
...
void TestBasicCreation() {
  ...
  for (const std::string name : {"blotto", "goofspiel", "kuhn_poker", "tiny_hanabi", "phantom_ttt", "matrix_rps",
                                 "leduc_poker"}) {
    std::cout << "RestrictedMashResponse: Testing " << name << std::endl;
...
void TestFixedPolicyGame() {
  // Check the RNR which automatically puts the strategy in the game as chance nodes
  // Setup
  Player fixed_player = 1;
  std::shared_ptr<const Game> game = LoadGameAsTurnBased("matrix_mp");
  std::shared_ptr<TabularPolicy> fixed_policy = std::make_shared<TabularPolicy>(*game);
  auto initial_state = game->NewInitialState();
  initial_state->ApplyAction(0);
  fixed_policy->SetStatePolicy(initial_state->InformationStateString(), {{0,1},{1,0}});
  // P 0.6 case when the resulting strategy is pure
  std::shared_ptr<const Game> rnr_game = ConvertToRNR(*game, fixed_player, 0.6, fixed_policy);
  algorithms::CFRPlusSolver solver(*rnr_game);
  for (int i = 0; i < 1000; i++) {
    solver.EvaluateAndUpdatePolicy();
  }
  const std::shared_ptr<Policy> average_policy = solver.AveragePolicy();
  auto player_two_policy = average_policy->GetStatePolicy("[Rnr: free]Current player: 1\nObserving player: 1. Non-terminal");
  for(int i = 0; i < player_two_policy.size();i++) {
    SPIEL_CHECK_FLOAT_NEAR(player_two_policy[i].second,i,0.001);
  }
  auto player_one_policy = average_policy->GetStatePolicy("Current player: 0\nObserving player: 0. Non-terminal");
  ...
  auto player_two_policy_two = average_policy_two->GetStatePolicy("[Rnr: free]Current player: 1\nObserving player: 1. Non-terminal");
  double check_policy[] = {1./6,5./6};
  for(int i = 0; i < player_two_policy_two.size();i++) {
    SPIEL_CHECK_FLOAT_NEAR(player_two_policy_two[i].second,check_policy[i],0.001);
  }
  auto player_one_policy_two = average_policy_two->GetStatePolicy("Current player: 0\nObserving player: 0. Non-terminal");
  check_policy[0] = check_policy[1] = 0.5;
  for(int i = 0; i < player_one_policy_two.size();i++) {
    SPIEL_CHECK_FLOAT_NEAR(player_one_policy_two[i].second,check_policy[i],0.001);
  }
}
