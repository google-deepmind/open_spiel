...
std::shared_ptr<const Game> Factory(const GameParameters &params) {
  return ConvertToRNR(*LoadGame(params.at("game").game_value()),
                      params.at("fixed_player").int_value(),
                      params.at("p").double_value());
}
...
class RestrictedNashResponseObserver : public Observer {
 public:
  ...
  void WriteTensor(const State &observed_state, int player,
    ...
    auto &state = open_spiel::down_cast<const RestrictedNashResponseState &>(observed_state);
    SPIEL_CHECK_GE(player, 0);
    SPIEL_CHECK_LT(player, state.NumPlayers());

    std::shared_ptr<const Game> original_game = state.GetOriginalGame();
    GameParameters params;
    std::shared_ptr<Observer> observer = original_game->MakeObserver(iig_obs_type_, params);
    // Observing player.
...
RestrictedNashResponseState::RestrictedNashResponseState(
    std::shared_ptr<const Game> game, std::unique_ptr<State> state, bool fixed, Player fixed_player,
    bool initial_state, double p, const std::shared_ptr<Policy> &fixed_policy)
    : State(std::move(game)), state_(std::move(state)), is_initial_(initial_state), fixed_(fixed), p_(p),
      fixed_player_(fixed_player), fixed_policy_(fixed_policy), use_fixed_policy_(fixed_policy) {}

Player RestrictedNashResponseState::CurrentPlayer() const {
  if (is_initial_) {
    return kChancePlayerId;
  } else {
    if (use_fixed_policy_ and fixed_ and state_->CurrentPlayer() == fixed_player_) {
      return kChancePlayerId;
    } else {
...
void RestrictedNashResponseState::DoApplyActions(const std::vector<Action> &actions) {
...
RestrictedNashResponseState::ChanceOutcomes() const {
  if (is_initial_) {
    ...
    } else if (use_fixed_policy_ and fixed_ and state_->CurrentPlayer() == fixed_player_) {
...
std::vector<Action> RestrictedNashResponseState::LegalActions(Player player) const {
  ...
  if (is_initial_) {
    ...
    if (use_fixed_policy_ and fixed_ and state_->CurrentPlayer() == fixed_player_) {
      // In other states if we exchanged fixed player nodes for chance node we return action for chance player
      if (player == kChancePlayerId) {
...
std::string RestrictedNashResponseState::ActionToString(Player player,
...
std::string RestrictedNashResponseState::InformationStateString(
    Player player) const {
  const auto &game = open_spiel::down_cast<const RestrictedNashResponseGame &>(*game_);
...
std::string RestrictedNashResponseState::ObservationString(Player player) const {
  const auto &game = open_spiel::down_cast<const RestrictedNashResponseGame &>(*game_);
...
std::shared_ptr<const Game> ConvertToRNR(
    const Game &game, Player fixed_player, double p, const std::shared_ptr<Policy>& fixed_policy) {
  return std::shared_ptr<const RestrictedNashResponseGame>(
      new RestrictedNashResponseGame(game.shared_from_this(), fixed_player, p, fixed_policy));
}
