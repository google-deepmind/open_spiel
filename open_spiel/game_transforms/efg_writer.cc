...
EFGWriter::EFGWriter(const Game& game, const std::string filename,
  ...
  if (game_.GetType().dynamics != GameType::Dynamics::kSequential) {
    SpielFatalError("EFGWriter only supports sequential games.");
  }
  if (game_.GetType().reward_model != GameType::RewardModel::kTerminal) {
    SpielFatalError("EFGWriter only supports terminal rewards.");
  }
  if (game_.GetType().chance_mode == GameType::ChanceMode::kSampledStochastic) {
    SpielFatalError("EFGWriter does not support sampled chance nodes.");
  }
}

void EFGWriter::Write() {
  std::ofstream efg_file;
  efg_file.open(filename_);
  efg_file << "EFG 2 R";
  ...
  Write(efg_file, state.get());
  efg_file.close();
}

void EFGWriter::Write(std::ostream &f, State* state) {
  if (state->IsTerminal()) {
    f << "t \"\" ";
