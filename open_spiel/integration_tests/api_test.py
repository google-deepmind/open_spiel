...
class PartialEnforceAPIConventionsTest(parameterized.TestCase):
  ...
  def test_observations_are_consistent_with_info_states(self, game_name):
    ...
    # We need to run tens of thousands of rollouts for this test
    # to be meaningful. Some games are however quite slow, so I benchmarked
    # time in secs for 5 rollouts, and put it into a table here. We will do
    # as many rollouts as we can within specified time limit.
    perf_factor = dict(
        coordinated_mp=0.0006759166717529297,
        cursor_go=0.18346023559570312,
        deep_sea=0.0008356571197509766,
        first_sealed_auction=0.0006062984466552734,
        go=10.2967848777771,  # very slow
        goofspiel=0.05947065353393555,
        havannah=9.846575021743774,  # very slow
        hex=3.1608517169952393,
        kuhn_poker=0.0009539127349853516,
        leduc_poker=0.0031385421752929688,
        oshi_zumo=0.0085296630859375,
        othello=0.5855708122253418,
        pentago=0.07950377464294434,
        quoridor=13.477667808532715,  # very slow
        tic_tac_toe=0.0036101341247558594,
        tiny_bridge_2p=0.08948898315429688,
        tiny_bridge_4p=0.007530927658081055,
        tiny_hanabi=0.0006313323974609375,
        y=12.566664934158325,  # very slow
        catch=0.0029778480529785156,
        chess=1.7099366188049316,
        cliff_walking=0.00045561790466308594,
        connect_four=0.028658628463745117,
    )
    # Run (very roughly!) for this many seconds. This very much depends on the
    # machine the test runs on.
    time_limit = 10
    # Compute (very roughly!) corresponding number of rollouts.
    rollouts = math.floor(time_limit * (1 / (perf_factor[game_name] / 5))) \
      if game_name in perf_factor else 100
    ...
    aoh_histories = collections.defaultdict(lambda: set())  # aoh -> states
    is_histories = collections.defaultdict(lambda: set())  # info_states -> states
    ...
    def show_error(histories, player, dump_collections=True):
      ...
      info_states_str = "\n".join([
        str(info_state) for info_state in info_states])
    ...
    def collect_and_test_rollouts(player):
      ...
      # todo: we want to check terminals for consistency too, but info state
      # string is not defined there and neither are observations by design.
      while not state.is_terminal():
