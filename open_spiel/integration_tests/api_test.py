...
class EnforceAPIOnPartialTreeBase(parameterized.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.some_states = set(
        sample_some_states.sample_some_states(
            cls.game,
            max_rollouts=-1,  # Limit only by time.
  ...
  def _check_partition_consistency(self,
      partition_callbacks: Dict[str, Callable[[int, pyspiel.State], str]],
      partition_relations: List[Tuple[str, Relation, str]]):

    for player in range(self.game.num_players()):
      states_representations = collections.defaultdict(dict)
      partitioned_sets = {name: collections.defaultdict(set)
        ...
        for state in self.some_states:
          ...
          if relation == Relation.EQUALS:
            if histories_x != histories_y:
              self.assertEqual(histories_x, histories_y,
                               self._show_error(player, state, x, relation, y,
                                                rep_x, rep_y, histories_x,
                                                histories_y))
          elif relation == Relation.SUBSET_OR_EQUALS:
            if not histories_x.issubset(histories_y):
              self.assertTrue(histories_x.issubset(histories_y),
                              self._show_error(player, state, x, relation, y,
                                               rep_x, rep_y, histories_x,
                                               histories_y))
          else:
  ...
  def _show_error(self, pl, state, x, relation, y,
      rep_x, rep_y, histories_x, histories_y):
    s = f"Observation consistency failed.\n" \
        f"It should hold that partition of '{x}' is {relation} of '{y}'\n" \
        f"Problem occurred while evaluating action history " \
        f"'{state.history_str()}' for player {pl}.\n" \
        f"The state representation is:\n{state}\n" \
        + ("-" * 80) + \
        f"\nDetailed information:\n" \
        f"The set of histories that have the same '{x}':\n{histories_x}\n" \
        f"The set of histories that have the same '{y}':\n{histories_y}\n" \
        f"The set difference is:\n{histories_y - histories_x}\n" \
        f"The problematic {x} is '{rep_x}'\n" \
        f"The problematic {y} is '{rep_y}'\n"
    return s

  def test_observation_consistency(self):
    if self.game_type.dynamics != pyspiel.GameType.Dynamics.SEQUENTIAL:
      return  # TODO(sustr): support also other types of games.
...
def _create_test_case_classes():
  ...
  for game_name in _GAMES_TO_TEST:
    ...
    new_class = type(
        "EnforceAPIPartialTree_{}_Test".format(game_name),
