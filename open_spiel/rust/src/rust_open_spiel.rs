...
impl State {
    ...
    pub fn action_to_string(&self, player: i32, action: i64) -> String {
        let mut length = 0;
        let c_buf: *mut c_char =
            unsafe { StateActionToString(self.state, player, action, &mut length) };
        convert_and_free_cstring(c_buf, length)
    }
    ...
    pub fn observation_string(&self) -> String {
        let mut length = 0;
        let c_buf: *mut c_char = unsafe { StateObservationString(self.state, &mut length) };
        convert_and_free_cstring(c_buf, length)
    }

    pub fn information_state_string(&self) -> String {
        let mut length = 0;
        let c_buf: *mut c_char = unsafe { StateInformationStateString(self.state, &mut length) };
        convert_and_free_cstring(c_buf, length)
    }
...
impl Game {
    pub fn new(game_name: &str) -> Self {
        ...
        Self { game: unsafe { LoadGame(game_name.as_ptr()) } }
    }

    pub fn new_with_parameters(parameters: &GameParameters) -> Self {
        Self { game: unsafe { LoadGameFromParameters(parameters.params) } }
    }
...
impl Bot {
    pub fn step(&mut self, state: &State) -> i64 {
        unsafe { BotStep(self.bot, state.state) }
    }

    pub fn inform_action(&mut self, state: &State, player_id: i32, action: i64) {
        unsafe { BotInformAction(self.bot, state.state, player_id, action) };
    }

    pub fn restart(&mut self) {
        unsafe { BotRestart(self.bot) };
    }
}

impl Drop for Bot {
    fn drop(&mut self) {
        unsafe { DeleteBot(self.bot) }
    }
}
...
pub fn create_bot_by_name(
    bot_name: &str,
    game: &Game,
    player_id: i32,
    params: &GameParameters,
) -> Bot {
    let bot_name = CString::new(bot_name).unwrap();
    let bot = unsafe {
        BotRegistererCreateByName(bot_name.as_ptr(), game.game, player_id, params.params)
    };
    Bot { bot }
}
