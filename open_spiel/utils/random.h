...
class RandomMT: public Random {
 ...
 public:
  explicit RandomMT(unsigned long seed)
      : generator_(std::mt19937(seed)) {}
  explicit RandomMT(const std::mt19937 &generator)
      : generator_(generator) {}
  double RandomUniform() override final;
};
...
class RandomFixedSequence: public Random {
  ...
  int position_ = 0;
 public:
  // "Always return value" constructor.
  explicit RandomFixedSequence(double value)
      : RandomFixedSequence(std::vector<double>{value}) {}

  // "Always return zero" constructor.
  explicit RandomFixedSequence() : RandomFixedSequence(0.) {}

  // Return values from this specified list.
  explicit RandomFixedSequence(std::vector<double> values)
      : values_(move(values)) {}
