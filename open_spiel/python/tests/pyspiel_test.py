...
class PyspielTest(parameterized.TestCase):
    ...
    def test_registered_names_contains_expected_games(self):
        ...
        if (
            os.environ.get("OPEN_SPIEL_BUILD_WITH_HANABI", "OFF") == "ON"
            and "hanabi" not in expected
        ):
            expected.append("hanabi")
        if (
            os.environ.get("OPEN_SPIEL_BUILD_WITH_ACPC", "OFF") == "ON"
            and "universal_poker" not in expected
        ):
        ...
        self.assertContainsSubset(
            expected_subset=remaining_games, actual_set=_FULLY_OPTIONAL_PYTHON_GAMES
        )
    ...
    def test_default_loadable(self):
        ...
        if (
            os.environ.get("OPEN_SPIEL_BUILD_WITH_ACPC", "OFF") == "ON"
            and "repeated_poker" not in expected
        ):
            expected.append("repeated_poker")
    ...
    def test_registered_game_attributes(self):
        ...
        self.assertEqual(
            game_list["kuhn_poker"].dynamics, pyspiel.GameType.Dynamics.SEQUENTIAL
        )
        self.assertEqual(
            game_list["kuhn_poker"].chance_mode,
            pyspiel.GameType.ChanceMode.EXPLICIT_STOCHASTIC,
        )
        self.assertEqual(
            game_list["kuhn_poker"].information,
            pyspiel.GameType.Information.IMPERFECT_INFORMATION,
        )
        self.assertEqual(
            game_list["kuhn_poker"].utility, pyspiel.GameType.Utility.ZERO_SUM
        )
    ...
    def test_create_game(self):
        ...
        self.assertEqual(
            game_info.information, pyspiel.GameType.Information.IMPERFECT_INFORMATION
        )
    ...
    def test_game_parameters_from_string_with_subgame(self):
        self.assertEqual(
            pyspiel.game_parameters_from_string("foo(x=2,y=true,subgame=bar(z=False))"),
            {"name": "foo", "x": 2, "y": True, "subgame": {"name": "bar", "z": False}},
        )
    ...
    def test_game_parameters_to_string_with_options(self):
        self.assertEqual(
            pyspiel.game_parameters_to_string({"name": "foo", "x": 2, "y": True}),
            "foo(x=2,y=True)",
        )
    ...
    def test_game_type(self):
        ...
        self.assertEqual(
            game_type.chance_mode, pyspiel.GameType.ChanceMode.DETERMINISTIC
        )
    ...
    def test_record_batched_trajectories(self):
        for game_name in ["kuhn_poker", "leduc_poker", "liars_dice"]:
            ...
            pyspiel.record_batched_trajectories(
                game,
                policies,
    ...
    @parameterized.parameters(
        ...
        ("python_pokerkit_wrapper_acpc_style", "python_pokerkit_wrapper_acpc_style(),"),
...
class StructApiTest(absltest.TestCase):
    ...
    def test_game_observation_struct_property(self):
        ...
        self.assertIs(self.game.ObservationStruct, self.ttt.TicTacToeObservationStruct)
