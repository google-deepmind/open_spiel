...
class PolicyGradient(rl_agent.AbstractAgent):
  ...
  def __init__(self,
    ...
    self._net_torso = simple_nets.MLPTorso(info_state_size,
                                           self._layer_sizes)
  ...
  def _initialize(self):
    ...
    self._savers = [
        ("torso", tf.train.Saver(self._net_torso.variables)),
        ("policy_head", tf.train.Saver(self._policy_logits_layer.variables))
    ]
    if self._loss_class.__name__ == "BatchA2CLoss":
