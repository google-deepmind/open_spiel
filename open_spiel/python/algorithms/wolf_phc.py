...
class WoLFPHC(rl_agent.AbstractAgent):
  ...
  def _hill_climbing(self, info_state, legal_actions):
    ...
    greedy_actions = [
        action for action in legal_actions if self._q_values[info_state][action] == greedy_q
    ]
    if len(greedy_actions) == len(legal_actions):
      return

    deltas = {action: min(self._cur_policy[info_state][action], self._cur_delta_value/(
        len(legal_actions) - len(greedy_actions))) for action in legal_actions}

    delta_greedy = sum(
        [deltas[action] for action in legal_actions if action not in greedy_actions])/len(greedy_actions)

    deltas = {action: -deltas[action]
              if action not in greedy_actions else delta_greedy for action in legal_actions}
    new_policy = np.array(
        [self._cur_policy[info_state][action] + deltas[action] for action in legal_actions])
