...
class MultiagentQTest(absltest.TestCase):
  ...
  def test_rps_run(self):
    ...
    nashqlearner0 = MAQLearner(
        0, 2, [env.game.num_distinct_actions()] * 2, TwoPlayerNashSolver())

    nashqlearner1 = MAQLearner(
        1, 2, [env.game.num_distinct_actions()] * 2, TwoPlayerNashSolver())

    for _ in range(1000):

      time_step = env.reset()
      nashqlearner0.restart_episode(), nashqlearner1.restart_episode()
      actions = [None, None]
      actions = [nashqlearner0.step(time_step, actions).action,
    ...
    with self.subTest("correct_rps_strategy"):
      ...
      learner0_strategy, learner1_strategy = nashqlearner0.step(
          time_step, actions).probs, nashqlearner1.step(time_step, actions).probs
      np.testing.assert_array_almost_equal(
          np.asarray([1/3, 1/3, 1/3]), learner0_strategy.reshape(-1), decimal=4)
      np.testing.assert_array_almost_equal(
          np.asarray([1/3, 1/3, 1/3]), learner1_strategy.reshape(-1), decimal=4)
