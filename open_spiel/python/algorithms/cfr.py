...
class _InfoStateNode(object):
  ...
  cumulative_regret: Dict[int, float] = attr.ib(factory=lambda: collections.defaultdict(float))
  # Same as above for the cumulative of the policy probabilities computed
  # during the policy iterations
  cumulative_policy: Dict[int, float] = attr.ib(factory=lambda: collections.defaultdict(float))


def _apply_regret_matching_plus_reset(info_state_nodes: Dict[str, _InfoStateNode]):
...
def _update_current_policy(current_policy: policy.TabularPolicy,
...
def _update_average_policy(average_policy: policy.TabularPolicy,
                           info_state_nodes: Dict[str, _InfoStateNode]):
...
class _CFRSolverBase(object):
  ...
  def __init__(self, game: pyspiel.Game, alternating_updates: bool,
               linear_averaging: bool, regret_matching_plus: bool):
    # pyformat: disable
    """Initializer.
  ...
  def _compute_counterfactual_regret_for_player(self, state: pyspiel.State,
...
def _regret_matching(cumulative_regrets: Dict[int, float],
