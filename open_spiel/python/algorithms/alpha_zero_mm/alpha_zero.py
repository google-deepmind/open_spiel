...
from open_spiel.python.algorithms.alpha_zero_mm import mcts
from open_spiel.python.algorithms.alpha_zero_mm import minimax
from open_spiel.python.algorithms.alpha_zero_mm import evaluator as evaluator_lib
...
class Trajectory(object):
  """A sequence of observations, actions, and the outcomes."""

  def __init__(self):
    self.states = []
    self.returns = None

  def add(self, information_state, policy, action):
    self.states.append((information_state, policy, action))
...
def _init_bot(config, game, evaluator_, evaluation):
  ...
  return minimax.minimaxBot(game,
                            evaluator=evaluator_) # HERE +
...
def _play_game(logger, game_num, game, bots, temperature, temperature_drop):
  ...
  while not state.is_terminal():
    ...
    value, action = bots[state.current_player()].alpha_beta_search(state=state, maximum_depth=30)
    
    # print('\n\n\n\n', value, str(action), '\n\n\n\n')
    # if len(actions) >= temperature_drop:
    #   pass
    # else:
    #   action = np.random.choice(game.num_distinct_actions())

    trajectory.states.append(TrajectoryState(
        state.observation_tensor(), state.current_player(),
        state.legal_actions_mask(), action, policy, value)) # HERE +
...
def evaluator(*, game, config, logger, queue):
  ...
  logger.print("Initializing bots")
  az_evaluator = evaluator_lib.AlphaZeroEvaluator(game, model)
  random_evaluator = mcts.RandomRolloutEvaluator() # HERE + no change, I think it's okaay to use mcts rollouts as evaluator
