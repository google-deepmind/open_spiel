...
class _InfoStateNode(object):
  ...
  # An array representing the preceeding actions played upto this information state
  history = attr.ib()
  ...
  #A dictionary mapping each deviation to their "y values" for the current iteration
...
class _EFRSolverBase(object):
  ...
  def _initialize_info_state_nodes(self, state, history, path_indices):
    ...
    Args:
    state: The current state in the tree traversal. This should be the root node
    when we call this function from the EFR solver.
    history: an arrays of the preceeding actions taken prior to the state for each player.
    path_indices: a 3d array [player number]x[preceeding state]x[legal actions for state, 
    index of the policy for this state in TabularPolicy].
    """
    ...
    for action in info_state_node.legal_actions:
      ...
      self._initialize_info_state_nodes(state.child(action), new_history, new_path_indices)
  ...
  def _update_current_policy(self, state, current_policy):
    ...
    Note that the function is called recursively (first call should be the root). Additionally, 
    to update the strategy for a given state we require the (t+1)th strategy for all prior states.  

    Args:
    state: the state of which to update the strategy.
    current_policy: the (t+1)th strategy that is being recursively computed, see the function
...
class EFRSolver(_EFRSolver):
  ...
  def __init__(self, game, deviations_name):
    ...
    #external_only = True leads to a shortcut in the computation of the next timesteps strategy from the regrets
    external_only = False
    deviation_sets = None
...
def return_blind_action(num_actions, history, _):
  ...
  Args:
    ...
    history: an array containing the prior actions played by the `player` to reach the information set.
  Returns:
...
def return_cs_partial_sequence(num_actions, history, prior_legal_actions):
  ...
  Args:
    num_actions: the integer of all actions that can be taken at that information set
    history: an array containing the prior actions played by the `player` to reach the information set.
    prior_legal_actions: a 2d array containing the legal actions for each preceeding state.
  Returns:
  ...
  external = return_all_external_modified_deviations(
      num_actions, external_memory_weights, prior_legal_actions, prior_actions_in_memory)
...
def generate_all_action_permutations(current_stem, remaining_actions):
  """
  Args:
     current_stem: the prior sequence of actions to be completed by the remaining actions
     remaining_actions: a 2d array of [subsequent states]x[possible actions]
  Returns:
     An array with each element being the current stem joined with a possible permuation of remaining actions
  """
...
class LocalDeviationWithTimeSelection(object):
  """"
  Comprised of a swap transformation that will be applied at the current information state, a memory weighting
  which describes the which actions are remembered and the memory action history (prior_memory_actions) that is remembered.
  Note that the "memory action history" might not equal the history in the case of some deviation types (e.g tips deviations).
  """
  ...
  def __init__(self, target, source, num_actions, prior_actions_weight, prior_memory_actions,
    ...
    use_unmodified_history: a boolean used to indicate whether the provided memory_actions are the same as 
    the information state it was derived from.
    """
  ...
  def deviate(self, strategy):
    """
    Returns the strategy array given by deviating according to the 'self.local_swap_transform.matrix_transform' matrix.
    Args:
  ...
  def player_deviation_reach_probability(self, prior_possible_action_probabilities):
    ...
    player played from the start of the game to this state. This is assuming that they play
    with their current strategy with the deviation applied. 
    Args:
       prior_possible_action_probabilities: a 2d array of length 
          [player's history]x[number of actions at that state]. These are the current strategies of
          the player, from start to end of their history. 
    Returns:
...
def return_all_internal_modified_deviations(num_actions, possible_prior_weights, possible_prior_memory_actions, prior_memory_actions):
  deviations = []
  for prior_actions_weight in possible_prior_weights:
    try:
...
def return_all_external_modified_deviations(num_actions, possible_prior_weights, possible_prior_memory_actions, prior_memory_actions):
  deviations = []
  for prior_actions_weight in possible_prior_weights:
    try:
      modification_index = np.where(prior_actions_weight == 0)[0][0]
...
class LocalSwapTransform(object):
  ...
  def __eq__(self, other: object) -> bool:
    if self.source_action == other.source_action and self.target_action == other.target_action and self.actions_num == other.actions_num:
      return True
    else:
      return False
