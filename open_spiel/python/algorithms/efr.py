...
class EFRSolver(_EFRSolver):
  def __init__(self, game, deviations_name):
    ...
    elif deviations_name == "bhv" or deviations_name == "single target behavioural" or deviations_name =="behavioural":
...
def return_blind_action(num_actions, history, _):
    ...
    Returns:
      an array of LocalDeviationWithTimeSelection objects that represent all Blind Action deviations that are realizable at the
      information set. 
    """
...
def return_blind_CF(num_actions, history, _):
    ...
    Args:
      ...
      history: an array containing the prior 
    Returns:
      an array of LocalDeviationWithTimeSelection objects that represent all Blind CF deviations that are realizable at the
      information set. 
    """
    memory_weights = [None]
...
def return_blind_partial_sequence(num_actions, history, _):
    """
    Returns an array of all Blind Partial Sequence deviations (BPS) with respect to an information set
    Args:
      num_actions: the integer of all actions that can be taken at that information set
      history: an array containing the prior 
    Returns:
      an array of LocalDeviationWithTimeSelection objects that represent all BPS deviations that are realizable at the
      information set. 
    """
    prior_actions_in_memory = history
    memory_weights = [None]
...
def return_cf_partial_sequence(num_actions, history, _):
    """
    Returns an array of all Counterfactual Partial Sequence deviations (CFPS) with respect to an information set
    Args:
      num_actions: the integer of all actions that can be taken at that information set
      history: an array containing the prior 
    Returns:
      an array of LocalDeviationWithTimeSelection objects that represent all CFPS deviations that are realizable at the
      information set. 
    """
    prior_actions_in_memory = history
    memory_weights = [None]
...
def return_cs_partial_sequence(num_actions, history, prior_legal_actions):
    ...
    Returns:
      an array of LocalDeviationWithTimeSelection objects that represent all Casual Partial Sequence deviations that are realizable at the
      information set. 
    """
    prior_actions_in_memory = history
...
def return_twice_informed_partial_sequence(num_actions, history, prior_legal_actions):
    """
    Returns an array of all Twice Informed Partial Sequence (TIPS) deviations with respect to an information set.
    Args:
      num_actions: the integer of all actions that can be taken at that information set
      history: an array containing the prior 
      prior_legal_actions: an array containing the index in .... that 
...
def return_all_internal_modified_deviations(num_actions,  possible_prior_weights, possible_prior_memory_actions, prior_memory_actions, _):
    ...
    for prior_actions_weight in possible_prior_weights:
        try:
            modificationIndex = np.where(prior_actions_weight == 0)[0][0]
        except:
            modificationIndex = 0
        if modificationIndex == len(prior_memory_actions):
            for target in range(num_actions):
                for source in range(num_actions):
                    if not source == target:
                        deviations.append(LocalDeviationWithTimeSelection(target, source, num_actions, prior_actions_weight, prior_memory_actions, False))
        else:
            previous_action = prior_memory_actions[modificationIndex]
            for alt_action in possible_prior_memory_actions[modificationIndex]:
                prior_memory_actions[modificationIndex] = alt_action
                for target in range(num_actions):
                    for source in range(num_actions):
                        if not source == target:
                            deviations.append(LocalDeviationWithTimeSelection(target, source, num_actions, prior_actions_weight, prior_memory_actions.copy(), False))
    ...
    for prior_actions_weight in possible_prior_weights:
        ...
        if modificationIndex == len(prior_memory_actions):
            ...
            for alt_action in possible_prior_memory_actions[modificationIndex]:
                prior_memory_actions[modificationIndex] = alt_action
                for target in range(num_actions):
                    deviations.append(LocalDeviationWithTimeSelection(target, target, num_actions, prior_actions_weight, prior_memory_actions.copy(), True))
                prior_memory_actions[modificationIndex] = previous_action
    return deviations
