...
from open_spiel.python.algorithms.alpha_zero.utils import api_selector, AVIALABLE_APIS, linen_to_nnx
import pyspiel
...
flags.DEFINE_string("path", None, "A path (directory) for a pretrained model chekpoint")
flags.DEFINE_integer("checkpoint_step", None, "A step for a pretrained model chekpoint")
flags.DEFINE_string("config_path", None, "Filename for the a training config.json")
...
flags.DEFINE_bool("visualise", False, "Use it to draw a treescope layout of the model")
...
def build_model() -> nnx.Module | linen.Module:
  ...
  model = api_selector(config.get("nn_api_version", FLAGS.nn_api_version)).Model.build_model(
...
def main(unused_argv):
  ...
  if FLAGS.visualise:
    nnx_model = linen_to_nnx(model._model) if not isinstance(model, nnx.Module) else model
