...
def learner(
  *,
  ...
  def learn(step: int | str) -> tuple[str, Losses]:
  ...
  for step in itertools.count(1):
    ...
    logger.print(
      f"Buffer size: {len(replay_buffer)}. States seen: {replay_buffer.total_seen}"
    )
...
def alpha_zero(config: Config) -> None:
  ...
  print(
    f"Model type: {config.nn_model}(width={config.nn_width}, depth={config.nn_depth})"
  )
