...
class AlphaZeroModel(nn.Module):
  ...
  @nn.compact
  def __call__(self, observations: chex.Array, training: bool) -> tuple[chex.Array, chex.Array]:
...
class Model:
  ...
  def build_model(
    ...
    if not decouple_weight_decay:
      ...
      optimiser = optax.adamw(learning_rate=learning_rate, weight_decay=weight_decay, mask=mask_only_biases)
    ...
    def update_step_fn(state, observations, legals_mask, policy_targets, value_targets):
      ...
      def loss_fn(params, batch_stats, observations, legals_mask, policy_targets, value_targets):
        ...
        @functools.partial(jax.vmap, in_axes=(0, 0, 0, 0), out_axes=(0, None), axis_name="batch")
