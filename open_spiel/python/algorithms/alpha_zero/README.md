...
This is a pure python implementation of the AlphaZero algorithm. It's based on
`flax` library for neural networks in `jax`. For more information, please take a
look at the
...
> [!NOTE] Before running the code, you might want to install additional
> requirements (see `scripts/python_extra_deps.sh`). `jax` has to be
> re-installed to match the available hardware, see:
...
To visualise the results, using the [analysis.py](./analysis.py): `Bash python
analysis.py --path absolute/path/to/checkpoint/dir`
...
Currently, the framework supports two APIs: * currently stable `flax.linen` that
encompasses functional paradigm * still experimental, but soon to be stable
`flax.nnx` which is much closer to the OOP paradigm. We mostly focus on the
refactoring of the existing solution, but there're some additional opportunities
provided by the `flax.nnx` lifted tranforms:
[examples](https://github.com/google/flax/blob/main/examples/nnx_toy_examples/)

## Changelog:

1.  Fully rewritten `tensorflow` model to `jax`, supported in 2 APIs, that can
    be used interchangeably
2.  Rewritten utils like replay buffer and configuation classes to support the
