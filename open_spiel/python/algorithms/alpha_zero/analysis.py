...
flags.DEFINE_string("path", None, "Where to find config.json and learner.jsonl.")
flags.DEFINE_enum(
...
def plot_histogram_named(ax, x, data, data_col, normalized=True):
  ...
  df = prepare(data, {name: data_col + ["counts", i] for i, name in enumerate(names)})
  if normalized:
...
def plot_data(config, data):
  ...
  cols = ["hit_rate", "usage"]
  df = prepare(data, {v: ["cache", v] for v in cols})
  ax = subplot(num_rows, num_cols, 7, title="Cache usage and hit rate.", ylim=(0, 1.05))
  for y in cols:
    df.plot(ax=ax, x=x, y=y)
  ...
  ax = subplot(num_rows, num_cols, 9, title="Inference batch size + stddev + min/max")
