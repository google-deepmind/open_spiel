...
def _update_average_policy(average_policy, info_state_nodes, br_list):
  ...
  """
    for info_state, info_state_node in info_state_nodes.items():
        ...
        for i in range(len(state_policy)):
            if i not in info_state_node.legal_actions:
                state_policy[i] = 0

        # print(state_policy, info_state)
        # print(info_state_policies_sum.items())
        ...
        # print(state_policy, info_state, 'after')
...
class _CFRSolverBase(object):
  ...
  """
    ...
    def __init__(self, game, br_list, alternating_updates, linear_averaging,
    ...
    """
        ...
        # done
        # go through current policy, make non br actions 0, renormalize
    ...
    """
        ...
        if info_state_node is None:
            ...
            for br in self._br_list:
                try:
                    br_policy = _policy_dict_at_state(br[current_player], state)
                    br_policies.append(br_policy)
                except:
                    # print('error when initializing')
                    # print(state, 'state')
                    # have br policy be arbitrary
                    # br_policy = [0 for _ in range(len(legal_actions))]
                    # br_policy[0] = 1
                    # br_policies.append(br_policy)
                    pass
            ...
            for action in legal_actions:
                ...
                for br_policy in br_policies:
                    # print(br_policy, 'br policy')
                    # print(action, 'action')
                    if br_policy[action] == 1:
                        br_actions.append(action)
                        action_in_brs = True
                # if not action_in_brs:
                    # print(info_state, action, current_player, 'not in br')
                # else:
                #     print(info_state, action, current_player, 'in br')

            br_actions = list(set(br_actions))

            info_state_node = _InfoStateNode(
                # legal_actions=legal_actions,
                legal_actions=br_actions,
        ...
        for action in info_state_node.legal_actions:
            ...
            # print(action)
            # print(state)
            # print(info_state_node.legal_actions)
            self.num_infostates_expanded += 1
            self._initialize_info_state_nodes(state.child(action))
    ...
    Returns:
      ...
      players.
    """
        _update_average_policy(self._average_policy, self._info_state_nodes, self._br_list)
