"""
Example running strategy exploration with fast and slow oracle.
...
#TODO: Tuning hyperparams for policy gradient method.
flags.DEFINE_string("oracle_type", "PG", "Choices are DQN, PG (Policy "
...
def gpsro_looper(env, oracle, oracle_list, agents):
  ...
  for gpsro_iteration in range(FLAGS.gpsro_iterations):
    ...
    if env.game.get_type().dynamics == pyspiel.GameType.Dynamics.SEQUENTIAL:
      ...
      # TODO: Here NashConv may not be defined correctly.
...
def main(argv):
  ...
  with tf.Session() as sess:
    ...
    oracle_list.append(slow_oracle)
    oracle_list.append(fast_oracle)

    gpsro_looper(env, slow_oracle, oracle_list, agents=None)
