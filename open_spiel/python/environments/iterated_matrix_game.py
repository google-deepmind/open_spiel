"""
This module implements a generic environment for iterated normal form games with
automatic vectorization. Along with the environment, it also provides
pre-defined factory functions for common games such as the iterated prisoners
dilemma and the iterated matching pennies.
"""
...
class IteratedMatrixGame(Environment):
  """
  Environment for iterated normal form games. Supports automatic vectorization.
  """
  ...
  def __init__(self,
    ...
    self._t = 0
    self._actions = np.arange(np.prod(
      self.action_spec()['num_actions'])).reshape(
        *[payoff_matrix.shape[p] for p in range(self._num_players)])
  ...
  def observation_spec(self):
    return dict(info_state=tuple([
      np.prod(self._payoff_matrix.shape[:-1]) + 1 +
      (1 if self._include_remaining_iterations else 0)
    ] for _ in range(self._num_players)),
                legal_actions=tuple([
                  self._payoff_matrix.shape[p] for p in range(self._num_players)
                ]),
                current_player=())
  ...
  def action_spec(self):
    return dict(
      ...
      max=tuple(
        [self._payoff_matrix.shape[p] - 1 for p in range(self._num_players)]),
      dtype=int)
  ...
  def step(self, actions: np.ndarray):
    ...
    s1 = self.one_hot(self._actions[tuple(actions.T)] + 1,
                      n=np.max(self._actions) + 2)
    s2 = self.one_hot(self._actions[tuple(actions[..., ::-1].T)] + 1,
                      n=np.max(self._actions) + 2)
    rewards = [
      np.squeeze(p)
      for p in np.split(payoffs, indices_or_sections=self._num_players, axis=1)
    ]
    ...
    if self._include_remaining_iterations:
      info_state = np.concatenate([
        ...
        np.full((self._batch_size, 1), fill_value=remaining_iters)
                                  ...
                                  axis=-1)
    return TimeStep(observations=dict(
      info_state=info_state,
      legal_actions=np.array(
        [[np.arange(self.action_spec()['num_actions'][p])] * self._batch_size
  ...
  def reset(self):
    self._t = 0
    info_state = np.zeros((self.num_players, self._batch_size,
...
def IteratedPrisonersDilemma(iterations: int, batch_size=1):
  return IteratedMatrixGame(payoff_matrix=np.array([[[-1, -1], [-3, 0]],
                                                    [[0, -3], [-2, -2]]]),
...
def IteratedMatchingPennies(iterations: int, batch_size=1):
  return IteratedMatrixGame(payoff_matrix=np.array([[[1, -1], [-1, 1]],
                                                    [[-1, 1], [1, -1]]]),
                            iterations=iterations,
                            batch_size=batch_size,
                            include_remaining_iterations=False)
