"""
Example that trains two agents using either LOLA (Foerster et al., 2017) or 
LOLA-DiCE (Foerster et al., 2018) on iterated matrix games. Hyperparameters are 
...
from open_spiel.python.environments.iterated_matrix_game import \
...
flags.DEFINE_integer('critic_mini_batches', 1,
                     'Number of minibatches for critic.')
...
flags.DEFINE_string('correction_type', 'lola',
                    'Either "lola", "dice" or None.')
flags.DEFINE_integer('n_lookaheads', 2,
                     'Number of lookaheads for LOLA correction.')
flags.DEFINE_float('correction_max_grad_norm', None,
                   'Maximum gradient norm of LOLA correction.')
flags.DEFINE_float('discount', 0.96, 'Discount factor.')
flags.DEFINE_integer('policy_update_interval', 1,
                     'Number of critic updates per before policy is updated.')
flags.DEFINE_integer('eval_batch_size', 1024, 'Random seed.')
flags.DEFINE_bool('use_jit', False,
                  'If true, JAX jit compilation will be enabled.')
flags.DEFINE_bool('use_opponent_modelling', True,
                  'If false, ground truth opponent weights are used.')
flags.DEFINE_integer('opp_policy_mini_batches', 8,
                     'Number of minibatches for opponent policy.')
flags.DEFINE_float('opponent_model_learning_rate', 0.3,
                   'Learning rate for opponent model.')
flags.DEFINE_bool('debug', False, 'If true, debug mode is enabled.')


def get_action_probs(agent: OpponentShapingAgent,
                     game: str) -> List[typing.Dict[str, typing.Any]]:
  """
    Returns the probability of cooperation and a string representation for each
    state.
    Args:
        agent: The agent.
...
def collect_batch(env: Environment, agents: List[OpponentShapingAgent],
                  eval_mode: bool) -> List[TimeStep]:
  """
...
def make_agent(
    key: jax.random.PRNGKey, player_id: int, env: Environment,
    networks: Tuple[hk.Transformed, hk.Transformed]) -> OpponentShapingAgent:
  """
  ...
  return OpponentShapingAgent(
    ...
    critic_discount=
    0,  # Predict only the immediate reward (only for iterated matrix games)
...
def make_agent_networks(
    num_states: int, num_actions: int) -> Tuple[hk.Transformed, hk.Transformed]:
  """
  ...
  def value_fn(obs):
    w = hk.get_parameter('w', [num_states], init=jnp.zeros)  # @pylint: disable=invalid-name
...
def make_env(game: str, iterations: int, batch_size: int) -> Environment:
  """
    Creates an environment (either iterated prisoners dilemma or iterated
    matching pennies).
    Args:
...
def setup_agents(env: Environment,
                 rng: hk.PRNGSequence) -> List[OpponentShapingAgent]:
  """
  ...
  for player_id in range(env.num_players):
    networks = make_agent_networks(num_states=info_state_shape[player_id][0],
                                   num_actions=num_actions[player_id])
    agent = make_agent(key=next(rng),
                       player_id=player_id,
                       env=env,
                       networks=networks)
    agents.append(agent)
...
def main(_):
  ...
  env = make_env(iterations=FLAGS.game_iterations,
                 batch_size=FLAGS.batch_size,
                 game=FLAGS.game)
