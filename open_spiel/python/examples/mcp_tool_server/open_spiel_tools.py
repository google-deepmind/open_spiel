...
class GameSession:
  ...
  def maybe_step(self):
    """Steps game state until it is the player's turn or the game is over."""
    while self.state.current_player() != self.player_id:
      if self.state.is_simultaneous_node():
        raise NotImplementedError(
            'Simultaneous nodes are not supported.'
        )
      ...
      if self.state.is_chance_node():
        ...
        # Note: the opponent plays a uniform random actions.
        # TODO(author5): Allow the user to specify the opponent policy.
...
class OpenSpielTools:
  ...
  def list_games(self) -> list[str]:
    ...
    """
    return pyspiel.registered_concrete_names()
  ...
  def mcts_action_chooser(self, session_id: str, num_simulations: int):
    ...
    if (
        state.get_game().get_type().information
