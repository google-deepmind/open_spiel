...
def run_experiment(num_players, env, payoffs, centralized):
    ...
    for i in range(num_runs):
        ...
        agents = [
            tabular_qlearner.QLearner(player_id=idx,
        ...
        for cur_episode in range(training_episodes):
            time_step = env.reset()
            # Find cur_state for logging. See lewis_signaling.cc for info_state details
        ...
        for s in range(num_states):
            ...
            if centralized:
                ...
                info_state0[3 + s] = 1.0
            m, _ = agents[0]._epsilon_greedy(str(info_state0),
                                             np.arange(num_messages), 0)
            ...
            if centralized:
                ...
                info_state1[3 + m] = 1.0
            a, _ = agents[1]._epsilon_greedy(str(info_state1),
                                             np.arange(num_states), 0)
    ...
    if FLAGS.plot:
        ...
        def plot_scalars(scalars,
            ...
            for i, scalar in enumerate(scalars):
                xs = np.arange(
                    scalar.shape[1 - repetition_axis]) * FLAGS.log_interval
        ...
        for i, cp in enumerate(converge_point_list):
            plot_confusion_matrix(cp.astype(np.int),
                                  title="Final policy (Tabular {})".format(
                                      labels[i]))
