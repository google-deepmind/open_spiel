...
PYBIND11_MODULE(pyspiel, m) {
  ...
  game_type
      ...
      .def("pretty_print", [](const GameType& value) {
        return GameTypeToString(value);
      })
      .def("__repr__", [](const GameType& gt) {
        return "<GameType '" + gt.short_name + "'>";
      })
      .def("__eq__", [](const GameType& value, GameType* value2) {
        return value2 && GameTypeToString(value) == GameTypeToString(*value2);
      })
