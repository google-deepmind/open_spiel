...
def make_iterated_matrix_game(game: str,
                              iterations=5,
                              batch_size=8) -> rl_environment.Environment:
...
def make_agent_networks(
    num_actions: int) -> Tuple[hk.Transformed, hk.Transformed]:
...
def run_agents(agents: typing.List[OpponentShapingAgent],
...
class LolaPolicyGradientTest(parameterized.TestCase, absltest.TestCase):
  ...
  def test_run_game(self, game_name):
    batch_size = 8
    iterations = 5
    env = make_iterated_matrix_game(game_name,
                                    batch_size=1,
    ...
    agents = [
      OpponentShapingAgent(player_id=i,
                           opponent_ids=[1 - i],
                           seed=key,
                           ...
                           discount=0.96,
                           use_jit=False) for i in range(2)
    ]
