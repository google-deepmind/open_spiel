"""
JAX implementation of LOLA (Foerster et al., 2018) and LOLA-DiCE 
...
class TransitionBatch:  # pylint: disable=too-few-public-methods
  """
    A transition batch is a collection of transitions. Each item in the batch is
    a numpy array.
    """
...
UpdateFn = typing.Callable[[TrainState, TransitionBatch],
                           typing.Tuple[TrainState, typing.Dict]]


def get_minibatches(batch: TransitionBatch,
                    num_minibatches: int) -> typing.Iterator[TransitionBatch]:
  """
    Returns an iterator over minibatches of the given batch.
    Args:
...
def get_critic_update_fn(
  ...
  def update(train_state: TrainState, batch: TransitionBatch):
    ...
    state = TrainState(
      ...
      critic_params={
        **train_state.critic_params, agent_id: critic_params
      },
      critic_opt_states={
        **train_state.critic_opt_states, agent_id: opt_state
      },
    )
...
def get_dice_update_fn(
  ...
  def rollout(params, other_params):
    ...
    step = env.reset()
    batch_size = (step.observations['batch_size']
                  if 'batch_size' in step.observations else 1)
    while not step.last():
      ...
      a_2 = get_action(other_params, s_2,
                       jax.random.split(next(rng), num=batch_size))
    ...
    def outer_update(params, opp_params, agent_id, opp_id):
      ...
      for _ in range(n_lookaheads):
        ...
        other_grad = jax.grad(dice_objective)(
          ...
          values=critic_network.apply(train_state.critic_params[opp_id],
      ...
      values = critic_network.apply(train_state.critic_params[agent_id],
                                    trajectories['states'][0])
      loss = dice_objective(
        params=params,
  ...
  def update(train_state: TrainState,
             batch: TransitionBatch) -> typing.Tuple[TrainState, typing.Dict]:
    """
        Updates the policy parameters in train_state. If lola_weight > 0, the
        correction term according to Foerster et al. will be applied.
        Args:
    ...
    updates, opt_state = optimizer(grads,
                                   train_state.policy_opt_states[agent_id])
    policy_params = optax.apply_updates(train_state.policy_params[agent_id],
                                        updates)
    train_state = TrainState(
      policy_params={
        **train_state.policy_params, agent_id: policy_params
      },
      policy_opt_states={
        **train_state.policy_opt_states, agent_id: opt_state
      },
...
def get_lola_update_fn(
  ...
  def flat_params(
    params,
  ) -> typing.Tuple[typing.Dict[str, jnp.ndarray], typing.Dict[
  ...
  def lola_correction(train_state: TrainState,
                      batch: TransitionBatch) -> hk.Params:
    """
    ...
    g_t = vmap(vmap(compute_returns))(r_t=r_t,
                                      v_t=values,
                                      discount_t=jnp.full_like(r_t, gamma))
    ...
    def cross_term(a_t, o_t, r_t):
      ...
      log_probs, grads = grad_log_pi(params[agent_id], agent_id, a_t[agent_id],
                                     o_t[agent_id])
      opp_logrpobs, opp_grads = grad_log_pi(params[opp_id], opp_id, a_t[opp_id],
                                            o_t[opp_id])
      ...
      for t in range(0, len(a_t[agent_id])):
        ...
        cross_term += (discounted_reward * jnp.outer(grads[t], opp_grads[t]) *
  ...
  def policy_loss(params, agent_id, batch):
    ...
    returns = vmap(rlax.lambda_returns)(r_t=r_t,
                                        v_t=values,
                                        discount_t=discount,
                                        lambda_=jnp.ones_like(discount))
    adv_t = returns - values
    loss = vmap(rlax.policy_gradient_loss)(logits_t=logits_t,
                                           a_t=a_t,
                                           adv_t=adv_t,
                                           w_t=jnp.ones_like(adv_t))
    return loss.mean()
  ...
  def update(train_state: TrainState,
    ...
    """
        Updates the policy parameters in train_state. If lola_weight > 0, the
        correction term by Foerster et al. will be applied.
        Args:
            train_state: the agent's train state.
            batch: a transition batch
    ...
    updates, opt_state = optimizer(policy_grads,
                                   train_state.policy_opt_states[agent_id])
    policy_params = optax.apply_updates(train_state.policy_params[agent_id],
                                        updates)
    train_state = TrainState(
...
def get_opponent_update_fn(
  ...
  def loss_fn(params, batch: TransitionBatch):
    ...
    log_probs = vmap(vmap(loss, in_axes=(None, 0, 0)),
                     in_axes=(None, 0, 0))(params, batch.info_state[agent_id],
                                           batch.action[agent_id])
  ...
  def update(train_state: TrainState,
    ...
    loss = 0
    for mini_batch in get_minibatches(batch, num_minibatches):
      loss, policy_grads = jax.value_and_grad(loss_fn)(policy_params,
                                                       mini_batch)
...
class OpponentShapingAgent(rl_agent.AbstractAgent):
  """
    Opponent Shaping Agent that uses either LOLA or LOLA-DiCE to influence the
    parameter updates of the opponent policies.
    """
  ...
  def __init__(
    ...
    self._policy_opt = optax.chain(
      ...
      if clip_grad_norm else optax.identity(),
    ...
    for opponent in opponent_ids:
      ...
      self._policy_update_fns[opponent] = (jax.jit(opp_update_fn)
                                           if use_jit else opp_update_fn)
      self._critic_update_fns[opponent] = (jax.jit(opp_critic_update_fn)
                                           if use_jit else opp_critic_update_fn)
  ...
  def get_value_fn(self) -> typing.Callable:
    ...
    def value_fn(obs: jnp.ndarray):
      ...
      return self._critic_network.apply(
        self.train_state.critic_params[self.player_id], obs).squeeze(-1)
  ...
  def get_policy(self, return_probs=True) -> typing.Callable:
    """
        ...
        Args:
            return_probs: if true, the policy returns a tuple
            (action, action_probs).
  ...
  def step(self, time_step: TimeStep, is_evaluation=False):
    """
        Produces an action and possibly triggers a parameter update. LOLA agents
        depend on having access to previous actions made by the opponent.
        Assumes that the field 'observations' of time_step contains a field
        'actions' and its first axis is indexed by the player id. Similar, the
        ...
        """
    do_step = (time_step.is_simultaneous_move() or
  ...
  def _init_train_state(self, info_state_size: chex.Shape):
    ...
    for agent_id in agent_ids:
      policy_params[agent_id] = self._pi_network.init(next(self._rng),
                                                      init_inputs)
  ...
  def _store_time_step(self, time_step: TimeStep, action: np.ndarray):
    ...
    self._step_counter += (time_step.observations['batch_size']
                           if 'batch_size' in time_step.observations else 1)
    ...
    if time_step.last():
      ...
      time_step.observations['values'] = jnp.stack([
        self._critic_network.apply(self.train_state.critic_params[id],
                                   jnp.array(obs[id])).squeeze(-1)
  ...
  def _train_step(self):
    """
        Updates the critic and the policy parameters. After the update, the data
        buffer is cleared.
        Returns: None
        """
  ...
  def _should_update(self) -> bool:
    """
        ...
        """
    return (self._step_counter >= self._batch_size *
            (self._num_learn_steps + 1) and self._episode_counter > 0)
  ...
  def _construct_episode_batches(
      self, transitions: typing.List[TransitionBatch]) -> TransitionBatch:
    """
        Constructs a list of transitions into a single transition batch
        instance.
        The fields 'info_state', 'rewards', 'legal_action_mask' and 'actions' of
        the produced transition batch have shape
        (num_agents, batch_size, sequence_length, *shape).
        The fields 'discount' and 'terminal' have shape
