...
Uses same CFR setting as open_spiel.python.algorithms.cfr._CFRSolverBase and the usability should be interchangable.

The results may slightly differ between these 2 versions due to rounding errors when computing regrets (rounding regrets smaller than epsilon to zero results in exactly the same results)

The algorithm performs well in short but wide games, with small amount of illegal actions and poorly in long games with a lot of illegal actions.
"""
...
def regret_matching(regret, mask):
  ...
  policy = jnp.where(total > 0.0, regret / total, 1.0 / jnp.sum(mask)) * mask
...
class JaxCFRConstants:
  players: int
  max_depth: int
  max_actions: int # This includes chance outcomes!  TODO: We could do this separately for each depth to make less computations
...
class JaxCFR:
  r"""Class for CFR and CFR+
    
  First it prepares all the structures in `init`, then it just reuses them within jitted function `jit_step`.
  """
  def __init__(self, game: pyspiel.Game, regret_matching_plus=True, alternating_updates=True, linear_averaging=True):
    self.game = game
  ...
  def init(self):
    ...
    def _traverse_tree(state, previous_info, depth, chance = 1.0):
      ...
      for a in state.legal_actions():
        ...
        next_history_temp[a] = len(depth_history_player[depth + 1]) if len(depth_history_player) > depth + 1 else 0
        
        _traverse_tree(new_state, new_info, depth + 1, new_chance)
            

    s = self.game.new_initial_state()
    _traverse_tree(s, PreviousInfo(tuple(0 for _ in range(players)), tuple(0 for _ in range(players)), tuple(0 for _ in range(players)), 0, 0), 0)
  ...
  def evaluate_and_update_policy(self):
    """Wrapper to step(), that ensures interchangability with open_spiel.python.algorithms.cfr._CFRSolverBase
    """
    self.step()
  ...
  def step(self):
    ...
    if self._alternating_updates:
      for player in range(self.constants.players):
        self.regrets, self.averages = self.jit_step(self.regrets, self.averages, averaging_coefficient, player)
      
    else:
      self.regrets, self.averages = self.jit_step(self.regrets, self.averages, averaging_coefficient, JAX_CFR_SIMULTANEOUS_UPDATE)
  ...
  def propagate_strategy(self, current_strategies):
    ...
    Args:
      current_strategies: Current strategies for all players, list[Float[Isets, Actions]]
    """ 
    realization_plans = [jnp.ones_like(current_strategies[pl]) for pl in range(self.constants.players)]

    for pl in range(self.constants.players):
      for i in range(0, self.constants.max_iset_depth[pl] + 1):
        realization_plans[pl] = jnp.where(self.constants.iset_action_depth[pl][..., jnp.newaxis] == i, current_strategies[pl] * realization_plans[pl].ravel()[self.constants.iset_previous_action[pl]][..., jnp.newaxis], realization_plans[pl])
    ...
    for i in range(self.constants.max_depth -2, -1, -1):
      ...
      for pl in range(self.constants.players):
        ...
        for pl2 in range(self.constants.players):
          if pl != pl2:
            regret = regret * realization_plans[pl2].ravel()[self.constants.depth_history_previous_action[pl2][i]][..., jnp.newaxis]
