...
class DeepNeurdModel(nn.Module):
  ...
  def __init__(
    ...
    layers_.extend([
        (
            rcfr.ResidualMLPBlock(  # pylint: disable=g-long-ternary
                num_hidden_units,
                num_hidden_units,
                num_hidden_factors,
                hidden_activation,
  ...
  def forward(self, x: torch.Tensor, autoencode: bool) -> torch.Tensor:
    ...
    Args:
      ...
      autoencode: Whether or not this is being called during training. If
        the constructor argument `autoencode` was `True`, then the output will
        contain the estimated regrets concatenated with a reconstruction of the
        input, otherwise only regrets will be returned.
