...
class EVAAgent(object):
  ...
  def step(self, time_step, is_evaluation=False):
    ...
    if (not is_evaluation and self._last_time_step is not None):
      ...
      infostate_embedding = self._embedding_network(self._info_state).detach()[0]
      ...
      if t < len(trajectories) - 1:
        for action in range(len(legal_actions)):
          ...
          if action == elem.action:
            ...
            self._agent.info_state = torch.Tensor(np.expand_dims(elem.info_state, axis=0))
            q_values_parametric = self._agent._q_network(self._agent.info_state).detach().numpy()
