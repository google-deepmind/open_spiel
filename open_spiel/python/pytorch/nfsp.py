...
class NFSP(rl_agent.AbstractAgent):
  ...
  def add_transition(
    self, time_step:  rl_environment.TimeStep, agent_output: rl_agent.StepOutput
  ) -> None:
    ...
    transition = Transition(
      info_state=np.asarray(
        time_step.observations["info_state"][self.player_id], dtype=np.float32
      ),
  ...
  def save(
    ...
    Args:
      ...
      save_optimiser (bool, optional): whether save only the optimiser
