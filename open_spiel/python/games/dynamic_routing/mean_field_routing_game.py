...
from open_spiel.python.games.dynamic_routing.dynamic_routing_game_utils import (
    Network, OriginDestinationDemand, NO_POSSIBLE_ACTION, INDEX_FIRST_ACTION,
...
_DEFAULT_NETWORK = Network({"bef_O": "O", "O": ["A"], "A": ["D"],
                            "D": ["aft_D"], "aft_D": []})
_DEFAULT_DEMAND = [OriginDestinationDemand("bef_O->O", "D->aft_D", 0, 100)]
...
class MeanFieldRoutingGame(pyspiel.Game):
    ...
    link they would like to go. At each chance node, the vehicle is assigned to
    a probability to exit its current road section based on the current volume
    on its road section (given by the distribution of players) and the exit
    ...
    Attributes inherited from GameInfo:
        ...
        num_player: the number of vehicles. Should be 1 as this mean field
    ...
    """
    network: Network
    _od_demand: List[OriginDestinationDemand]
    perform_sanity_checks: bool

    def __init__(
        self, params: Mapping[str, Any] = None,
        network: Network = None,
        od_demand: List[OriginDestinationDemand] = None,
        ...
        Args:
            ...
            od_demand: a list of the vehicle. Their origin and their
                destination should be road sections of the game. The number of
                vehicles in the list set the num_player attribute.
    ...
    def make_py_observer(self, iig_obs_type=None, params=None):
        ...
        return NetworkObserver(
            self.network.num_links(), self.max_game_length())
...
class MeanFieldRoutingGameState(pyspiel.State):
    ...
    Attributes:
      ...
      _init_distribution: probability at time 0 for the representative player
        ...
        _od_demand.
      _is_chance_init: boolean that encodes is the current node is the initial
        chance node.
      ...
      _vehicle_destination: the destination of each vehicle.
      _vehicle_final_travel_time: the travel times of each vehicle, the travel
      ...
      _vehicle_location: current location of the vehicle as a network
        road section.
      _vehicle_movement: Movement of the vehicle. Either the vehicle is moving
        to the next road section (True) either it is stuck in traffic on its
        current road section (False).
      _vehicle_without_legal_action: list of vehicles without legal actions at
    ...
    def assert_valid_action(self, action: int, road_section: str = None):
        ...
        num_distinct_actions. In case road_section is not None then it is test
        if the action correspond to going on a road section which is a successor
        of road_section.
        """
        assert isinstance(action, int), f"{action} is not a int."
        assert INDEX_FIRST_ACTION <= action < self.get_game(
            ).num_distinct_actions()
        if road_section is not None:
            new_road_section = self.get_game(
                ).network.get_road_section_from_action_id(action)
    ...
    def _legal_actions(self, player: pyspiel.PlayerId) -> List[int]:
        ...
        if self._vehicle_without_legal_action:
            ...
            return [NO_POSSIBLE_ACTION]
        if not self._vehicle_movement:
            return [NO_POSSIBLE_ACTION]
    ...
    def _apply_action(self, action: int):
        ...
        if self._player_id == pyspiel.PlayerId.CHANCE:
            ...
            if self._is_chance_init:
                ...
                self._vehicle_movement = bool(action)
            ...
            if not self._vehicle_without_legal_action:
                ...
                if self._vehicle_movement:
                    if self.get_game().perform_sanity_checks:
                        self.assert_valid_action(action, self._vehicle_location)
                    self._vehicle_location =\
    ...
    def get_location_as_int(self) -> int:
        ...
        if not hasattr(self, '_vehicle_location'):
            return -1
        start_node, end_node = _road_section_to_nodes(self._vehicle_location)
    ...
    def __str__(self) -> str:
        ...
        if hasattr(self, '_vehicle_location'):
            return self.state_to_str(
                ...
                vehicle_movement=self._vehicle_movement)
...
class NetworkObserver:
    ...
    def __init__(self, num_links: int, num_time: int):
