...
class Network:
    ...
    A network is basically a directed graph with volume delay function on each
    of its edges. Each vertex is refered as a string (for example "A") and each
    edge as a string f"{node1}->{node2}" (for example "A->B"). The network is
    created from a adjacency list. Each road section is mapped to a action index
    ...
    Attributes:
        _action_to_road_section: dictionary that maps action id to road
            section.
        _adjacency_list: adjacency list of the line graph of the road network.
        _node_have_position: bool that encodes if _node_position is not empty.
        _node_position: dictionary that maps node to couple of float encoding
            x and y position of the node. Empty if _node_have_position is
            False.
        _probability_to_exit_functions: dictionary of functions as string
            assigned to road sections. A function is a string that will be
            evaluated with x as parameter (for example '1/(1+x)').
        _road_section_to_action: dictionary that maps road section to action
            id.
    """
    _action_to_road_section: Dict[int, str]
    _adjacency_list: Dict[str, Iterable[str]]
    _node_have_position: bool = False
    _node_position: Dict[str, Tuple[float, float]] = None
    ...
    def __init__(
        ...
        probability_exit_function: Dict[str, str] = None
    ):
        ...
        if node_position:
            ...
            self._node_have_position = True
            self._node_position = node_position

        if probability_exit_function:
            assert set(probability_exit_function) == set(
                self._road_section_to_action), (
                "Exit functions are not defined for each road sections.")
            self._probability_to_exit_functions = probability_exit_function
        else:
        ...
        assert hasattr(self, '_node_have_position')
    ...
    def _create_movement_to_action_and_action_to_road_section(
        ...
        The dictionary that maps movement to action is used to define the
        action from a movement that a vehicle would like to do. The dictionary
        that maps an action to the destintion of the movement is used to move a
        vehicle that does an action to the destination of its movement.
        Args:
            network_adjacency_list: adjacency list of the network.
        Returns:
            movement_to_action: dictionary with key begin a movement for
                example "O->A" and value the action numbers. Action numbers are
                succesive integers indexed from 0.
        ...
        action_number = INDEX_FIRST_ACTION
        for origin, successors in self._adjacency_list.items():
            for destination in successors:
                ...
                if road_section in road_section_to_action:
                    ...
                    raise ValueError(
                        (f'{road_section} exists twice in the adjacency list. '
                         'The current network implementation does not enable '
                         'parallel links.'))
    ...
    def get_action_id_from_movement(
    ...
    def get_road_section_from_action_id(self, action_id: int) -> str:
        """Maps a road section to one action."""
    ...
    def check_list_of_vehicles_is_correct(self, vehicles: Iterable['Vehicle']):
        ...
        for vehicle in vehicles:
            if (vehicle.origin not in self._road_section_to_action
                or vehicle.destination not in self._road_section_to_action):
                raise ValueError(
                    f'Incorrect origin or destination for {vehicle}')
    ...
    def get_probability_to_exit(
        self, road_section: str, volume: float
    ) -> float:
    ...
    def return_list_for_matplotlib_quiver(
        ...
        Returns:
            o_xs, o_ys, d_xs, d_ys: list of the start x and y positions and of
                the end x and y postions of each road section. Each element of
                each list corresponds to one road section.
        """
        assert self._node_have_position, (
            "The network should have node positions in order to be plot.")
        o_xs = []
...
class Vehicle:
    ...
    def __str__(self):
        return (f"Vehicle with origin {self.origin}, destination "
                f"{self.destination} and departure time "
                f"{self._departure_time}.")
...
class OriginDestinationDemand(Vehicle):
    ...
    Attributes:
        ...
        counts: the number of vehicles with the origin, destination and
            departure time.
    """
    _counts: float
    ...
    def __init__(
        self, origin: str, destination: str,
        departure_time: float, counts: float
    ):
