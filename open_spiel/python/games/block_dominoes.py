...
_GAME_INFO = pyspiel.GameInfo(
  ...
  max_game_length=_MAX_GAME_LENGTH,  # deal: 14 chance nodes + play: 14 player nodes
  utility_sum=0.0)
...
class BlockDominoesObserver:
  ...
  def __init__(self, iig_obs_type, params):
    ...
    if iig_obs_type.private_info == pyspiel.PrivateInfoType.SINGLE_PLAYER:
      ...
      # 2 for the pips, and 1 to distinguish between (0,0) to empty slot for a tile.
      pieces.append(("hand", 21, (7, 3)))

    if iig_obs_type.public_info:
      if iig_obs_type.perfect_recall:
        # list of all played actions, each action is represented using 5 integers:
        # 2 for the played tile (0-6), 1 for the covered edge (0-6), 1 for which player (0/1),
  ...
  def set_from(self, state, player):
    ...
    if "actions_history" in self.dict:
      for i, action in enumerate(state.actions_history):
        ...
        self.dict["actions_history"][i][2] = action.edge if action.edge is not None else 0.
        self.dict["actions_history"][i][3] = action.player
        self.dict["actions_history"][i][4] = 1.
    ...
    if "last_action" in self.dict:
      if state.actions_history:
        ...
        self.dict["last_action"][2] = action.edge if action.edge is not None else 0.
