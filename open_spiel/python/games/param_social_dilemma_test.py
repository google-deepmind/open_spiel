...
from open_spiel.python.games import param_social_dilemma  # pylint: disable=unused-import
import pyspiel
...
class ParamSocialDilemmaTest(absltest.TestCase):
  ...
  def test_custom_game_length(self):
    game = pyspiel.load_game(
        "python_param_social_dilemma", {"max_game_length": 20})
  ...
  def test_dynamic_payoffs(self):
    game = pyspiel.load_game(
        "python_param_social_dilemma",
        {"dynamic_payoffs": True, "payoff_change_prob": 0.5, "players": 2})
  ...
  def test_custom_payoff_matrix(self):
    ...
    game = pyspiel.load_game(
        "python_param_social_dilemma",
        {"players": 2, "payoff_matrix": str(custom_payoff.tolist())})
    state = game.new_initial_state()
    state.apply_actions([0, 0])
  ...
  def test_game_progression(self):
    ...
    for _ in range(5):
      self.assertFalse(state.is_terminal())
      self.assertEqual(
          state.current_player(), pyspiel.PlayerId.SIMULTANEOUS)
  ...
  def test_observer(self):
    game = pyspiel.load_game(
        "python_param_social_dilemma", {"players": 2})
    state = game.new_initial_state()
