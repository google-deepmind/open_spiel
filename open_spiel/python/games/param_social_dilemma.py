...
class ParamSocialDilemmaGame(pyspiel.Game):
  """A parameterized N-player social dilemma game."""
    ...
    if payoff_matrix_str == "default":
      ...
      try:
        ...
        raise ValueError(
            ...
            f"Expected nested list as string. Error: {e}") from e
  ...
  def _create_default_payoff_matrix(self):
    """Creates the default payoff matrix for the social dilemma."""
    shape = tuple(
        [self._num_actions] * self._num_players + [self._num_players])
    payoff_matrix = np.zeros(shape)

    for idx in np.ndindex(shape[:-1]):
      num_cooperators = sum(
          1 for action in idx if action == Action.COOPERATE)
      for player in range(self._num_players):
        if idx[player] == Action.COOPERATE:
          payoff_matrix[idx][player] = (
              3.0 * num_cooperators / self._num_players)
        else:
          payoff_matrix[idx][player] = (
              5.0 * num_cooperators / self._num_players)
  ...
  def get_payoff(self, actions, timestep=0):  # pylint: disable=unused-argument
    """Returns the payoff for the given actions."""
...
class ParamSocialDilemmaState(pyspiel.State):
  """State for the parameterized social dilemma game."""

  def __init__(self, game):
    super().__init__(game)
    self._game_over = False
    self._timestep = 0
    self._rewards = np.zeros(game._num_players)  # pylint: disable=protected-access
    self._returns = np.zeros(game._num_players)  # pylint: disable=protected-access
  ...
  def _legal_actions(self, player):
    assert player >= 0
    return list(range(self.get_game()._num_actions))  # pylint: disable=protected-access
  ...
  def _apply_actions(self, actions):
    ...
    if game._reward_noise_std > 0:  # pylint: disable=protected-access
      noise = (np.random.randn(game._num_players)  # pylint: disable=protected-access
               * game._reward_noise_std)  # pylint: disable=protected-access
    ...
    if self._timestep >= game._max_game_length:  # pylint: disable=protected-access
      self._game_over = True

  def _action_to_string(self, player, action):  # pylint: disable=unused-argument
  ...
  def __str__(self):
    ...
    for action_entry in history:
      ...
      if player != pyspiel.PlayerId.SIMULTANEOUS:
        t = len([
            h for h in history[:history.index(action_entry)]
            if h.player == player
        ])
    ...
    for t in sorted(timesteps.keys()):
      actions_str = ",".join([
          timesteps[t].get(p, "?")
          for p in range(self.get_game()._num_players)  # pylint: disable=protected-access
      ])
...
class ParamSocialDilemmaObserver:
  """Observer for the parameterized social dilemma game."""
  ...
  def set_from(self, state, player):  # pylint: disable=unused-argument
    pass
