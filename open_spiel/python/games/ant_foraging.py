...
def _make_game_info(params):
    ...
    grid_size = params.get("grid_size", _DEFAULT_PARAMS["grid_size"])
    num_ants = params.get("num_ants", _DEFAULT_PARAMS["num_ants"])
...
_GAME_INFO = _make_game_info(_DEFAULT_PARAMS)
...
class AntForagingGame(pyspiel.Game):
    ...
    def __init__(self, params=None):
        ...
        self._pheromone_decay = params.get("pheromone_decay",
                                           _DEFAULT_PARAMS["pheromone_decay"])
...
class AntForagingState(pyspiel.State):
    ...
    def __init__(self, game):
        ...
        while len(self._food_positions) < self._num_food and attempts < 100:
            ...
            if (row, col) != self._nest_pos and (row, col) not in self._food_positions:
        ...
        self._pheromone_to_food = np.zeros((self._grid_size, self._grid_size),
                                           dtype=np.float32)
        self._pheromone_to_nest = np.zeros((self._grid_size, self._grid_size),
                                           dtype=np.float32)
    ...
    def _apply_action(self, action):
        ...
        if (not self._carrying_food[ant_idx] and
            (row, col) in self._food_positions):
    ...
    def __str__(self):
        ...
        lines = []
        lines.append(f"Turn {self._turn_count}/{self._max_turns}, "
