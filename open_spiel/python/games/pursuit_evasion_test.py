...
def _play_game(game, evader_bot=None):
  ...
  Args:
    ...
    evader_bot: The evader bot to use. If None, a uniform-random evader is used.

  Returns:
    A tuple of (terminal state, number of steps).
  """
...
class PursuitEvasionTest(absltest.TestCase):
  ...
  def test_deterministic_fixed_strategy(self):
    ...
    np.testing.assert_array_equal(
        state.observation_tensor(), state2.observation_tensor()
    )
