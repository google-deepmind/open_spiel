...
from open_spiel.python.games import vaqueritos  # pylint: disable=unused-import
import pyspiel
...
class IteratedPrisonersDilemmaTest(absltest.TestCase):
  ...
  def test_default_param(self):
    ...
    game = pyspiel.load_game("python_vaqueritos")
  ...
  def test_non_default_param_from_string(self):
    ...
    game = pyspiel.load_game(
        "python_vaqueritos(termination_probability=0.5)")
  ...
  def test_game_as_turn_based_via_string(self):
    ...
    game = pyspiel.load_game(
        "turn_based_simultaneous_game(game=python_vaqueritos())"
    )
