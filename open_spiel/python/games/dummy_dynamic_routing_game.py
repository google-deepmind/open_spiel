...
- three vehicles:
    ...
    - one vehicle trying to reach D from A
All the vehicle leave their origin at the beginning of the game.
Any vehicle at O can only go at A. Any vehicle at A can only go at D. At D,
vehicle cannot do any actions. When every vehicle has reached D the game is
finished.

For more general dynamic routing game, one can consider a more complex network,
with routing choice at intersections and probability to stay on a link as a
function of the number of cars on the link. Currently this is not done to test
the API.
"""
...
_NO_POSSIBLE_ACTION = 0
_INDEX_FIRST_ACTION = 1

_GAME_TYPE = pyspiel.GameType(
    short_name="python_dummy_dynamic_routing_game",
    long_name="Python Dummy Dynamic Routing Game",
...
def _create_movement_to_action_and_action_to_road_section(
    ...
    action_number = _INDEX_FIRST_ACTION
...
class DummyDynamicRoutingGame(pyspiel.Game):
    ...
    def new_initial_state(self) -> "DummyDynamicRoutingGameState":
        ...
        return DummyDynamicRoutingGameState(self)
...
class DummyDynamicRoutingGameState(pyspiel.State):
    ...
    def __init__(self, game: DummyDynamicRoutingGame):
    ...
    def assert_valid_action(self, action: int, location: str = ""):
        ...
        assert action >= _INDEX_FIRST_ACTION
    ...
    def _legal_actions(self, player: int) -> List[int]:
        ...
        Returns:
            list_legal_actions: a list of legal actions. If the game is
                ...
                destination or on a node without successors then
                _NO_POSSIBLE_ACTION is returned. Otherwise the list of
    ...
    def _apply_actions(self, actions: List[int]):
        ...
        a valid action, then the player will move to the successor link
...
pyspiel.register_game(_GAME_TYPE, DummyDynamicRoutingGame)
