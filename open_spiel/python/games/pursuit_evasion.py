...
class PursuitEvasionState(pyspiel.State):
  ...
  Attributes: pursuer_x, pursuer_y: current pursuer position. evader_x,
  evader_y: current evader position.
...
class PursuitEvasionObserver:
  ...
  def string_from(self, state, player):
    del player
    if self._iig_obs_type is not None and not self._iig_obs_type.public_info:
      return ""
    return str(state)
