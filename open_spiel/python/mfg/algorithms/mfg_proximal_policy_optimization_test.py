...
class PolicyTest(parameterized.TestCase):
    """Test the policy."""
    @parameterized.named_parameters(('python', 'mfg_crowd_modelling_2d',
    ...
    def test_train(self, name, setting):
        ...
        optimizer_actor = optim.Adam(agent.actor.parameters(), lr=1e-2, eps=1e-5)
        optimizer_critic = optim.Adam(agent.critic.parameters(), lr=1e-2, eps=1e-5)
        ...
        for _ in range(3):
            ...
            history = rollout(env, pop_agent, agent, args["num_episodes"], steps, device)
            # Calculate the advantage function
            adv, returns = calculate_advantage(args["gamma"], True, history["rewards"], 
        ...
        # Test convergence
        self.assertLessEqual(nashc, init_nashc)
