...
class NashRandomSearch(regret_minimizer.RegretMinimizer):
  """Nash Random Search Exploitability Minimizer.
  ...
  def step(self, T):  # pylint: disable=invalid-name
    ...
    while exploitability > self._rho_tol and n < max(
        T, self._regret_steps_per_step
    ):
      n += 1
      nu = self.get_nu()
      exploitability = self.compute_exploitability(nu)
