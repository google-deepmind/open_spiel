...
Reference:
    Algumaei, Talal, et al. "Regularization of the policy updates for stabilizing 
...
def learn(history, optimizer_actor, optimize_critic, agent,
           num_minibatches=5, update_epochs=5, itr_eps=0.05, eps_eps=0.2,
           alpha=0.5, ent_coef=0.01, max_grad_norm=5):
  ...
  for _ in range(update_epochs):
    for start in range(0, batch_size, mini_batch_size):
      ...
      _, newlogprob, entropy, new_value = agent.get_action_and_value(b_obs,
                                                      history["actions"][mb_inds])
      logratio = newlogprob - history["logprobs"][mb_inds]
      ratio = torch.exp(logratio)

      # Get the data under the iteration policy (the population policy)
      _, t_newlogprob, _, _ = agent.get_action_and_value(b_obs, history["t_actions"][mb_inds])
      ...
      pg_loss = - alpha * torch.min(pg_loss1, pg_loss2).mean() - (1-alpha) * \
      torch.min(t_pg_loss1, t_pg_loss2).mean()
      v_loss = F.smooth_l1_loss(new_value.reshape(-1), history["returns"][mb_inds]).mean()
