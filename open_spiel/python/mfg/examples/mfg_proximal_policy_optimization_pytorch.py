...
from open_spiel.python.mfg.algorithms.mfg_proximal_policy_optimization import Agent as mfg_ppo_agent
from open_spiel.python.mfg.algorithms.mfg_proximal_policy_optimization import learn
from open_spiel.python.mfg.algorithms.mfg_proximal_policy_optimization import calculate_advantage
from open_spiel.python.mfg.algorithms.mfg_proximal_policy_optimization import rollout
from open_spiel.python.mfg.algorithms.mfg_proximal_policy_optimization import calculate_explotability
from open_spiel.python.mfg.algorithms.mfg_proximal_policy_optimization import Policy as mfg_ppo_policy


FLAGS = flags.FLAGS

flags.DEFINE_integer("seed", default=0, help="Set a random seed.")
flags.DEFINE_string("exp_name",  default="mf-ppo", help="Set the name of this experiment")
flags.DEFINE_string("game_setting", default="crowd_modelling_2d_four_rooms", 
                    help="Set the game to benchmark options:(crowd_modelling_2d_four_rooms) \
                    and (crowd_modelling_2d_maze)")
flags.DEFINE_float("lr", default=1e-3, help="Learning rate of the optimizer")
flags.DEFINE_integer("num_episodes",  default=5, help="set the number of episodes \
                     of to collect per rollout")
flags.DEFINE_integer("update_episodes", default=20, help="set the number of episodes \
                     of the inner loop")
flags.DEFINE_integer("update_iterations", default=100, help="Set the number of global \
                     update steps of the outer loop")
flags.DEFINE_string("optimizer", default="Adam", help="Set the optimizer (Adam) or (SGD)")
flags.DEFINE_boolean("cuda", default=False, help="Use Gpu to run the experiment")

# MFPPO parameters
flags.DEFINE_float("gamma",  default=0.9, help="set discount factor gamma")
flags.DEFINE_integer("num_minibatches", default=5,  help="the number of mini-batches")
flags.DEFINE_integer("update_epochs",default=5, help="the K epochs to update the policy")
flags.DEFINE_float("clip_coef",  default=0.2, help="the surrogate clipping coefficient")
flags.DEFINE_float("ent_coef",  default=0.01, help="coefficient of the entropy")
flags.DEFINE_float("max_grad_norm", default=5, help="the maximum norm for the gradient clipping")
flags.DEFINE_float("alpha",  default=0.5, help="Set alpha to controll the iteration \
                   and epsiode policy updates")
