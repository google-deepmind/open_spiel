...
class GamePublicTree(GameTree, pygraphviz.AGraph):
  ...
  Attributes:
    ...
    public_to_base_decorator: Decorator function for arrows (between public tree
        and the world tree).
        Optional, default=`public_tree_viz.default_public_to_base_decorator`.
    kwargs: Keyword arguments passed on to `pygraphviz.AGraph.__init__` and
            to `pyspiel.treeviz.GameTree.__init__`.
  """
  ...
  def __init__(self,
    ...
    self.add_node(self.public_state_to_str(root_public_state),
    ...
    if target_public_to_base:
      ...
      if not found:
        logging.warning(
            f"Could not find target public state '{target_public_to_base}' "
