...
while episodeCount < 100000 {
  ...
  while !gameState.isTerminal {
    ...
    if isGreedy {  // If acting greedily
      ...
      actionToTake = qtable[gameState.informationState()]![Int.random(in: 0..<actionValueCount)].action
    }
    ...
    // (newInformationState)
    qtable[currentInformationState] = qtable[currentInformationState]!.map {
