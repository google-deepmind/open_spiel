...
public class GridMaze: GameProtocol {
  ...
  public var minUtility: Double { fatalError("Cannot be calcuated for GridMaze") }  /// TODO: Ok? It is not known
  public var maxUtility: Double { fatalError("Cannot be calcuated for GridMaze") }  /// TODO: Ok? It is not known
  ...
  public var maxGameLength: Int { fatalError("Cannot be calcuated for GridMaze") }  /// TODO: Ok? It is not known
...
//      )
  ...
  public var initialState: GameState {
    ...
    if slist.count == 1 {
      ...
      fatalError("GridMaze misconfigured: One (and only one) initial state is required and supported")
    }
  }
...
extension GridMaze.GameState {
  ...
  public mutating func apply(_ action: GridMaze.Action) {
    ...
    gridCell = gridCell2
    precondition(gridCell.canAttemptToBeEntered && gridCell.canBeEntered,
...
public struct GridCell {
  ...
  public var hasProbabilisticTarget: Bool { return entryJumpProbabilities.filter { $0.js != .Welcome }.count > 0 }

  init(oneWordDescription: String,
       reward: Double,
  ...
  func getTargetWJumpSpecification(takingAction: GridMaze.Action) -> GridCell {
    ...
    let targetCell2 = calculateTargeCellJumpSpecification(originCell: self,
  ...
  func calculateTargeCellJumpSpecification(originCell: GridCell,  // Neeeded for bounce-back
    targetCell: GridCell,
    js: JumpSpecification) -> GridCell {
    ...
    precondition(js == .Welcome || !rv.hasProbabilisticTarget,
  ...
  public func probeActionTargets(takingAction: GridMaze.Action) -> [(ProbRewardCell)] {
    precondition(canBeEntered && canAttemptToBeEntered && !isTerminal,
                 "Cannot probeActionWithProbabilities from this cell (\(row!),\(col!))")
    let targetCell = getTargetWOJumpSpecification(takingAction: takingAction)
    precondition(targetCell.canAttemptToBeEntered,
    ...
    let result: [ProbRewardCell] = targetCell.entryJumpProbabilities.map {
      let tc = calculateTargeCellJumpSpecification(originCell: self,
                                                   targetCell: targetCell,
                                                   js: $0.js)
...
extension GridMaze {
  ...
  public func printMazeAndTable(header: String,  // header string
  ...
  func getCellDefinitionStrs(printFullFloat: Bool) -> [[String]] {
    ...
    for row in maze {
      ...
      for cell in row {
        ...
        str = "\(cell.oneWordDescription)\(jsStr):\(double2Str(printFullFloat: printFullFloat, value: cell.rewardOnEnter))"
        if cell.isTerminal {
          str += ":T"
  ...
  func getTableAndPolicyStrs(
    ...
    for (ri, r) in maze.enumerated() {
      ...
      for ci  in 0..<r.count {
        ...
        var qs1 = "*"
        var qs2 = "*"
        if maze[ri][ci].canAttemptToBeEntered && maze[ri][ci].canBeEntered && !maze[ri][ci].isTerminal {
          qs1 = ""
          qs2 = ""
